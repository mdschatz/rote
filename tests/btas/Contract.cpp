...
void Usage() {
  std::cout << "./Contract <gridOrder> <gridDim0> <gridDim1> ... <ten1Order> "
               "<ten1Dim0> <ten1Dim1> ... <ten1Ind1> <ten1Ind2> ... "
               "<ten2Order> <ten2Dim0> <ten2Dim1> ... <ten2Ind1> <ten2Ind2> "
               "... \"<tensorDist>\"\n";
...
void ProcessInput(Unsigned argc, char **const argv, Params &args) {
  ...
  //        std::cerr << "Tensor distribution must be of same order as
  //        tensor\n"; Usage(); throw ArgException();
  //    }
}
...
template <typename T>
std::vector<LocalTest> CreateLocalTests(const Tensor<T> &A, const Tensor<T> &B,
                                        const IndexArray &AIndices,
                                        const IndexArray &BIndices,
                                        const IndexArray &CIndices) {
#ifndef RELEASE
...
template <typename T> void LocalContractTest(const Params &args) {
#ifndef RELEASE
  ...
  std::vector<LocalTest> localTests =
      CreateLocalTests(A, B, AIndices, BIndices, CIndices);
