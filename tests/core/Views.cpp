...
template <typename T> void PrintLocalView(const Tensor<T> &A) {
  Unsigned i;
...
template <typename T> void PrintView(const char *msg, const DistTensor<T> &A) {
  Unsigned i;
  const Unsigned order = A.Order();
  printf("    Info for: %s\n", msg);
  printf("      shape:");
...
template <typename T> void TestConstViews(DistTensor<T> &A) {
#ifndef RELEASE
  ...
  for (i = 0; i < order; i++) {
    ...
    while (AT.Dimension(mode) < A.Dimension(mode)) {
      printf("  iteration: %d\n", count);
      LockedRepartitionDown(AT, A0, A1,
                            /**/ /**/
                            AB, A2, mode, 1);
      /////////////////////////////////
      PrintView("A0", A0);
      ...
      count++;
      /////////////////////////////////
      SlideLockedPartitionDown(AT, A0, A1,
                               /**/ /**/
                               AB, A2, mode);
    }
  }
  ...
  //                printf("Splitting mode %d of dimension %d into shape (%d,
  //                %d)\n", modeToSplit, ATensor.Dimension(modeToSplit),
  //                newMode1Dim, newMode2Dim); LockedViewAsHigherOrder(AHO,
  //                ATensor, oldModes, splitShape); PrintLocalView(AHO);
  //            }
  //        }
  //    }
}

template <typename T> void TestNonConstViews(DistTensor<T> &A) {
#ifndef RELEASE
  ...
  // Perform lower/higher order view changes (only appl
ies to Local Tensors)
  Tensor<T> ATensor(order);
  ATensor = A.Tensor();
  Tensor<T> ALO(order), AHO(order);
  Unsigned startMode, nMergeModes;
  ...
  //                printf("Splitting mode %d of dimension %d into shape (%d,
  //                %d)\n", modeToSplit, ATensor.Dimension(modeToSplit),
  //                newMode1Dim, newMode2Dim); ViewAsHigherOrder(AHO, ATensor,
  //                oldModes, splitShape); PrintLocalView(AHO);
  //            }
  //        }
  //    }
}
...
template <typename T> void DistTensorTest(const Params &args, const Grid &g) {
#ifndef RELEASE
