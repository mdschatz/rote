...
void Usage() { std::cout << "./RedistCheckTest\n"; }

void RunTest(const Grid &g, const char *outDist, const char *inDist,
             const RedistType &redist, ModeArray &modes, bool shouldFail) {
  mpi::Comm comm = mpi::COMM_WORLD;
  const Int commRank = mpi::CommRank(comm);
  if (commRank == 0) {
    std::cout << outDist << " <-- " << inDist << std::endl;
  }
  Unsigned i;
  Unsigned tenOrder = 4;
  ...
  try {
    ...
    switch (redist) {
    case AG:
      B.AllGatherRedistFrom(A, modes);
      break;
    case A2A:
      B.AllToAllRedistFrom(A, modes);
      break;
    case Local:
      B.LocalRedistFrom(A);
      break;
    case GTO:
      B.GatherToOneRedistFrom(A, modes);
      break;
    //			case BCast:   B.BroadcastRedistFrom(A, modes); break;
    //			case Scatter: B.ScatterRedistFrom(A, modes); break;
    case Perm:
      B.PermutationRedistFrom(A, modes);
      break;
    // Following are ignored since are equivalent to non-reduce cases
    //			case RS:      B.ReduceScatterRedistFrom(A, modes);
    //break; 			case RTO:     B.ReduceToOneRedistFrom(A, modes);
    //break; 			case AR:      B.AllReduceRedistFrom(A, modes);
    //break;
    default:
      break;
    }
...
void CheckAGRedist(const Grid &g) {
  ...
  RunTest(g, "[(0,1),(2,3,9),(4),()]|(5,7)", "[(0,1),(2,3,9),(4),(6,8)]|(7,5)",
          redist, modes, false);

  // Check that failure occurs when non-dist modes are different
  RunTest(g, "[(0,1),(2,3,9),(4),()]|(5,7,8)",
          "[(0,1),(2,3,9),(4),(6,8)]|(7,5)", redist, modes, true);

  // Check that failure occurs when add suffix to mode dist
  RunTest(g, "[(0,1),(2,3,9),(4),(6,8)]|(5,7)", "[(0,1),(2,3,9),(4),()]|(7,5)",
          redist, modes, true);

  // Check that AG fails correctly (not suffix)
  RunTest(g, "[(0,1),(2,3,9),(4),()]|(5,7)", "[(0,1),(2,3,9),(8,4),(6)]|(7,5)",
          redist, modes, true);
...
void CheckA2ARedist(const Grid &g) {
  ...
  // Check that A2A partitioned modes work correctly (from one mode dists to
  // multiple)
  RunTest(g, "[(0,1),(2,3,9,8),(4,6),()]|(5,7)",
          "[(0,1),(2,3,9),(4),(6,8)]|(7,5)", redist, modes, false);

  // Check that A2A partitioned modes work correctly (from multiple mode dists
  // to one)
  RunTest(g, "[(0,1),(2,3,8,6),(4),(9)]|(5,7)",
          "[(0,1),(2,3),(4,6),(9,8)]|(7,5)", redist, modes, false);

  // Check "can't form partition" case fails
  RunTest(g, "[(0,1),(2,3),(4,8),(9,6)]|(5,7)",
          "[(0,1),(2,3),(4,6),(9,8)]|(7,5)", redist, modes, true);

  // Check non-dist mode changes case fails
  RunTest(g, "[(0,1),(2,3),(4),(9)]|(5,7,6,8)",
          "[(0,1),(2,3),(4,6),(9,8)]|(7,5)", redist, modes, true);

  // Check that A2A fails correctly (not suffix)
  RunTest(g, "[(0,1),(2,3,8,6),(4),(9)]|(5,7)",
          "[(0,1),(2,3),(4,6),(8,9)]|(7,5)", redist, modes, true);
...
void CheckLocalRedist(const Grid &g) {
  ...
  RunTest(g, "[(0,1),(2,3,9,6),(4),(8)]|(5,7)", "[(0,1),(2,3,9),(4),()]|(7,5)",
          redist, modes, false);
  ...
  RunTest(g, "[(0,1),(2,3,9),(8,4),(6)]|(5,7)", "[(0,1),(2,3,9),(4),()]|(7,5)",
          redist, modes, true);
...
void CheckPermuteRedist(const Grid &g) {
  ...
  RunTest(g, "[(9),(8),(7),(6,0,1,2)]|(5,4)", "[(9),(8),(7),(6,1,0,2)]|(5,4)",
          redist, modes, false);

  // Check that Permute works correctly (exchange to same dim modes)
  RunTest(g, "[(9,3),(8),(7,2),(6,0,1)]|(5,4)",
          "[(9,3),(8),(7,1),(6,0,2)]|(5,4)", redist, modes, false);

  // Check that Permute works correctly (exchange same dim modes)
  RunTest(g, "[(9,3),(8),(7,0),(6,2,1)]|(5,4)",
          "[(9,3),(8),(7,1,2),(6,0)]|(5,4)", redist, modes, false);

  // Check that Permute fails correctly (different dim exchange)
  RunTest(g, "[(9,3),(8),(7,1,0),(6,2)]|(5,4)",
          "[(9,3),(8),(7,1,2),(6,0)]|(5,4)", redist, modes, true);

  // Check that Permute works correctly (not suffix)
  RunTest(g, "[(9,3),(8),(7,0),(6,2,1)]|(5,4)",
          "[(9,3),(8),(7,1,2),(0,6)]|(5,4)", redist, modes, false);
...
void CheckGatherToOneRedist(const Grid &g) {
  ...
  RunTest(g, "[(9),(8),(7),(6)]|(5,4,2,0,1)", "[(9),(8),(7,0,2),(6,1)]|(5,4)",
          redist, modes, false);

  // Check that GTO fails correctly (not suffix)
  RunTest(g, "[(9,3),(8),(7),(6)]|(5,4,1,0,2)",
          "[(9,3),(8),(7,1),(0,2,6)]|(5,4)", redist, modes, true);

  // Check that GTO fails correctly (not added to non-dist)
  RunTest(g, "[(9,3),(8),(7),(6)]|(5,4,1)", "[(9,3),(8),(7,1),(6,0,2)]|(5,4)",
          redist, modes, true);
