...
void Usage() {
  std::cout << "./GenContractTest <gridShape> <distA> <indicesA> <distB> "
               "<indicesB> <distC> <indicesC> <m-dim> <k-dim> <n-dim>\n";
}
...
void RunTest(const Grid &g, const Params &args) {
  ...
  double alpha = 2.0;
  double beta = 3.0;
  GenContract(alpha, A, args.indicesA, B, args.indicesB, beta, C,
              args.indicesC);
  ...
  if (commRank == 0) {
    ...
    LocalContractAndLocalEliminate(alpha, checkA, args.indicesA, checkB,
                                   args.indicesB, beta, checkC, args.indicesC);
    double norm;
