...
public:
  // Main interface
  static void run(const DistTensor<T> &A, const std::string &indicesA,
...
private:
  // Struct interface
  static void
  setHadamardInfo(const DistTensor<T> &A, const IndexArray &indicesA,
  ...
  // Partition helpers
  static void
  runHelperPartitionBC(Unsigned depth, BlkHadamardStatCInfo &hadamardInfo,
                       const DistTensor<T> &A, const IndexArray &indicesA,
                       const DistTensor<T> &B, const IndexArray &indicesB,
                       DistTensor<T> &C, const IndexArray &indicesC);

  static void
  runHelperPartitionAC(Unsigned depth, BlkHadamardStatCInfo &hadamardInfo,
                       const DistTensor<T> &A, const IndexArray &indicesA,
                       const DistTensor<T> &B, const IndexArray &indicesB,
                       DistTensor<T> &C, const IndexArray &indicesC);

  // Internal interface
  static void run(const DistTensor<T> &A, const IndexArray &indicesA,
                  const DistTensor<T> &B, const IndexArray &indicesB,
                  DistTensor<T> &C, const IndexArray &indicesC,
                  const std::vector<Unsigned> &blkSizes, bool isStatC);

  // Local interface
  static void run(const Tensor<T> &A, const IndexArray &indicesA,
                  const Tensor<T> &B, const IndexArray &indicesB, Tensor<T> &C,
