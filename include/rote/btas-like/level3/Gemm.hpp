...
template <typename T>
inline void Gemm(T alpha, const Tensor<T> &A, const Tensor<T> &B, T beta,
                 Tensor<T> &C) {
#ifndef RELEASE
  ...
  blas::Gemm('N', 'N', m, n, k, alpha, A.LockedBuffer(), A.Stride(1),
             B.LockedBuffer(), B.Stride(1), beta, C.Buffer(), C.Stride(1));
}

template <typename T>
inline void Gemm(T alpha, const Tensor<T> &A, const Tensor<T> &B,
                 Tensor<T> &C) {
#ifndef RELEASE
  CallStackEntry entry("Gemm");
#endif
