...
void PackCommHelper(
  ...
  if (oldOrder == 0) {
    ...
    Unsigned srcStrideToMatch =
        modifiedData.srcBufStrides[0] * modifiedData.loopShape[0];
    Unsigned dstStrideToMatch =
        modifiedData.dstBufStrides[0] * modifiedData.loopShape[0];
    ...
    for (i = 1; i < oldOrder; i++) {
      if (modifiedData.srcBufStrides[i] == srcStrideToMatch &&
        ...
        srcStrideToMatch =
            modifiedData.srcBufStrides[i] * modifiedData.loopShape[i];
        dstStrideToMatch =
            modifiedData.dstBufStrides[i] * modifiedData.loopShape[i];
        mergeMode++;
      }
    }
  }
...
template <typename T> void Permute(const DistTensor<T> &A, DistTensor<T> &B) {
