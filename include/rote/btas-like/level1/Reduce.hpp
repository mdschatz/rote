...
template <typename T>
void LocalReduceElemSelect_merged(const Unsigned nReduceModes,
                                  const ObjShape &reduceShape,
                                  T const *const srcBuf,
                                  const std::vector<Unsigned> &srcStrides,
                                  T *const dstBuf,
                                  const std::vector<Unsigned> &dstStrides) {
...
template <typename T>
void LocalReduce(const Tensor<T> &A, Tensor<T> &B, const Permutation &permBToA,
  ...
  //    LocalReduceElemSelect_fast(nonReduceModes.size() - 1, nonReduceModes,
  //    reduceModes, A.Shape(), A.LockedBuffer(), srcStrides, B.Buffer(),
  //    dstStrides);
  std::vector<Unsigned> srcReduceStrides =
      FilterVector(srcStrides, reduceModes);
  std::vector<Unsigned> srcNonReduceStrides =
      NegFilterVector(srcStrides, reduceModes);
  std::vector<Unsigned> useSrcStrides = srcReduceStrides;
  useSrcStrides.insert(useSrcStrides.end(), srcNonReduceStrides.begin(),
                       srcNonReduceStrides.end());

  std::vector<Unsigned> dstReduceStrides =
      FilterVector(dstStrides, reduceModes);
  std::vector<Unsigned> dstNonReduceStrides =
      NegFilterVector(dstStrides, reduceModes);
  std::vector<Unsigned> useDstStrides = dstReduceStrides;
  useDstStrides.insert(useDstStrides.end(), dstNonReduceStrides.begin(),
                       dstNonReduceStrides.end());

  std::vector<Unsigned> srcReduceShape = FilterVector(shapeA, reduceModes);
  std::vector<Unsigned> srcNonReduceShape =
      NegFilterVector(shapeA, reduceModes);
  std::vector<Unsigned> useSrcShape = srcReduceShape;
  useSrcShape.insert(useSrcShape.end(), srcNonReduceShape.begin(),
                     srcNonReduceShape.end());

  LocalReduceElemSelect_merged(reduceModes.size(), useSrcShape,
                               A.LockedBuffer(), useSrcStrides, B.Buffer(),
                               useDstStrides);
}

template <typename T>
void LocalReduce(const Tensor<T> &A, Tensor<T> &B,
                 const ModeArray &reduceModes) {
...
template <typename T>
void LocalReduce(const DistTensor<T> &A, DistTensor<T> &B,
                 const ModeArray &reduceModes) {
  PROFILE_SECTION("LocalReduce");
  Unsigned i;
  ObjShape shapeB = A.Shape();
  for (i = 0; i < reduceModes.size(); i++)
    shapeB[reduceModes[i]] = Min(A.GetGridView().Dimension(reduceModes[i]),
                                 A.Dimension(reduceModes[i]));
  B.ResizeTo(shapeB);
  Zero(B);

  if (B.Participating()) {
    // Account for the local data being permuted
    Permutation permBToA = B.LocalPermutation().PermutationTo(
        A.LocalPermutation());
    LocalReduce(
        A.LockedTensor(), B.Tensor(), permBToA,
        FilterVector(A.LocalPermutation().InversePermutation().Entries(),
                     reduceModes));
  }
  PROFILE_STOP;
}
