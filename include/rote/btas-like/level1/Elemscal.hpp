...
template <typename T>
inline void ElemScal_fast(T const *const src1Buf, T const *const src2Buf,
                          T *const dstBuf, const ElemScalData &data) {
...
template <typename T>
void ElemScal(const DistTensor<T> &A, const DistTensor<T> &B,
              DistTensor<T> &C) {
