...
template <typename T>
inline void Print(const Tensor<T> &A, std::string title = "",
                  bool all = false) {
#ifndef RELEASE
...
template <typename T>
inline void PrintVector(const std::vector<T> &vec, std::string title = "",
                        bool all = false) {
  if (all || mpi::CommRank(MPI_COMM_WORLD) == 0) {
    std::ostream &os = std::cout;
    os << title << ":";
...
template <typename T>
inline void Print(const DistTensor<T> &A, std::string title = "") {
#ifndef RELEASE
  CallStackEntry entry("Print");
#endif
  std::ostream &os = std::cout;
...
template <typename T>
inline void PrintData(const Tensor<T> &A, std::string title = "",
                      bool all = false) {
  std::ostream &os = std::cout;
  os << title << std::endl;
...
template <typename T>
inline void PrintData(const DistTensor<T> &A, std::string title = "",
                      bool all = false) {
  std::ostream &os = std::cout;
  //    if( A.Grid().LinearRank() == 0 && title != "" ){
  os << title << std::endl;
...
template <typename T>
inline void PrintArray(const T *dataBuf, const ObjShape &shape,
                       const ObjShape strides, std::string title = "") {
  std::ostream &os = std::cout;
...
template <typename T>
inline void PrintArray(const T *dataBuf, const ObjShape &loopShape,
                       std::string title = "") {
  PrintArray(dataBuf, loopShape, Dimensions2Strides(loopShape), title);
}

inline void PrintPackData(const PackData &packData, std::string title = "") {
  std::ostream &os = std::cout;
  os << title << std::endl;
...
inline void PrintElemScalData(const ElemScalData &elemScalData,
                              std::string title = "", bool all = false) {
  std::ostream &os = std::cout;
  os << title << std::endl;
...
inline void PrintHadamardStatCData(const BlkHadamardStatCInfo &hadamardInfo,
                                   std::string title = "", bool all = false) {
  std::ostream &os = std::cout;
  os << title << std::endl;
...
inline void PrintHadamardScalData(const HadamardScalData &hadamardInfo,
                                  std::string title = "", bool all = false) {
  std::ostream &os = std::cout;
  os << title << std::endl;
...
inline void PrintRedistPlan(const TensorDistribution &startDist,
                            const RedistPlan &redistPlan,
                            std::string title = "") {
...
inline void PrintRedistPlanInfo(const RedistPlanInfo &redistPlanInfo,
                                std::string title = "", bool all = false) {
  std::ostream &os = std::cout;
  os << title << std::endl;
