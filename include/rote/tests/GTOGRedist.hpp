...
void CreateGTOGTestsHelper(const ModeArray &tenModesToRedist,
  ...
  ModeArray newTenModesToRedist(tenModesToRedist.begin(),
                                tenModesToRedist.end() - 1);
  ...
  for (i = 0; i < partialTests.size(); i++) {
    ...
    for (j = 0; j <= modeDistToRedist.size(); j++) {
      ModeArray thisRedistModes(modeDistToRedist.end() - j,
                                modeDistToRedist.end());
      ModeArray newCommModes = partialCommModes;
      newCommModes.insert(newCommModes.end(), thisRedistModes.begin(),
                          thisRedistModes.end());
      ...
      newTenDist[order].insert(newTenDist[order].end(), thisRedistModes.begin(),
                               thisRedistModes.end());
      RedistTest newTest;
...
std::vector<RedistTest> CreateGTOGTests(const TensorDistribution &distA) {
  Unsigned i;
