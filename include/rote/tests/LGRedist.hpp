...
void CreateLGTestsHelper(const ModeArray &modesToMove,
...
std::vector<RedistTest> CreateLGTests(const Unsigned &gridOrder,
                                      const TensorDistribution &distA) {
  Unsigned i;
  ...
  std::set_difference(gridModes.begin(), gridModes.end(), usedGridModes.begin(),
                      usedGridModes.end(), back_inserter(unusedGridModes));

  std::vector<ModeArray> redistModesGroups;
  for (i = 1; i < order; i++) {
    std::vector<ModeArray> newRedistModesGroups =
        AllCombinations(unusedGridModes, i);
