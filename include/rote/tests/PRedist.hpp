...
void CreatePTestsSinkHelper(const ModeArray &modesToMove,
                            const ModeArray &sinkModesGroup, const Grid &g,
  ...
  if (modesToMove.size() == 0) {
    for (i = 0; i < partialTests.size(); i++) {
      ...
      bool isValid = true;
      for (j = 0; j < order; j++) {
        if (prod(FilterVector(g.Shape(), partialDist[j])) !=
            prod(FilterVector(g.Shape(), distA[j]))) {
          isValid = false;
          break;
        }
      }
  ...
  for (k = 0; k < modesToMove.size(); k++) {
    ...
    CreatePTestsSinkHelper(newModesToMove, sinkModesGroup, g, distA,
                           newPartialTests, fullTests);
  }
}

void CreatePTestsSrcHelper(const ModeArray &srcModesGroup,
                           const ModeArray &sinkModesGroup, const Grid &g,
                           const TensorDistribution &distA,
                           const std::vector<RedistTest> &partialTests,
                           std::vector<RedistTest> &fullTests) {
  ...
  if (srcModesGroup.size() == 0) {
    for (i = 0; i < partialTests.size(); i++) {
      ...
      CreatePTestsSinkHelper(partialCommModes, sinkModesGroup, g, distA,
  ...
  CreatePTestsSrcHelper(newTenModesToRedist, sinkModesGroup, g, distA,
                        newPartialTests, fullTests);
}

std::vector<RedistTest> CreatePTests(const Grid &g,
                                     const TensorDistribution &distA) {
  Unsigned i;
  ...
  for (i = 0; i < srcTenModesGroups.size(); i++) {
    ...
    CreatePTestsSrcHelper(srcModesGroup, tensorModes, g, distA, partialTests,
                          ret);
  }
  return ret;
}
