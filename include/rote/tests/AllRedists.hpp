#pragma once
#ifndef ROTE_TESTS_ALLREDISTS_HPP
#define ROTE_TESTS_ALLREDISTS_HPP
...
template <typename T> void Set(Tensor<T> &A) {
...
template <typename T> void SetWith(DistTensor<T> &A, const T *buf) {
  Unsigned order = A.Order();
  Location loc(order, 0);
...
template <typename T> bool CheckResult(const DistTensor<T> &A) {
#ifndef RELEASE
  ...
  bool shouldbeParticipating =
      !AnyPositiveElem(FilterVector(myGridLoc, dist[order]));
  //    printf("shouldBeParticipating: %s\n", (shouldbeParticipating ? "true" :
  //    "false"));
  ...
  if (!shouldbeParticipating) {
    ...
    const Permutation permCheckToA =
        DeterminePermutation(check.LocalPermutation(), A.LocalPermutation());
...
template <typename T>
bool CheckReduceResult(const DistTensor<T> &A, const DistTensor<T> &orig,
...
void AllCombinationsHelper(const ModeArray &input, Unsigned arrPos, Unsigned k,
                           Unsigned prevLoc, ModeArray &piece,
                           std::vector<ModeArray> &combinations) {
  Unsigned i;
...
#endif // ifndef ROTE_TESTS_ALLREDISTS_HPP
