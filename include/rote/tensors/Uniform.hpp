...
template <typename T>
inline void MakeUniform(Tensor<T> &A, T center = 0, BASE(T) radius = 1) {
...
template <typename T>
inline void Uniform(Tensor<T> &A, const ObjShape &shape, T center = 0,
                    BASE(T) radius = 1) {
#ifndef RELEASE
...
template <typename T>
inline Tensor<T> Uniform(const ObjShape &shape, T center = 0,
                         BASE(T) radius = 1) {
  Tensor<T> A(shape);
  MakeUniform(A, center, radius);
  return A;
...
template <typename T> struct MakeUniformHelper {
  static void Func(DistTensor<T> &A, T center, BASE(T) radius) {
    ...
    // BCast data
    mpi::Comm comm =
        A.GetCommunicatorForModes(A.GetGridView().FreeModes(), A.Grid());
...
template <typename T>
inline void MakeUniform(DistTensor<T> &A, T center = 0, BASE(T) radius = 1) {
#ifndef RELEASE
  CallStackEntry cse("Uniform");
#endif
...
template <typename T>
inline void Uniform(DistTensor<T> &A, const ObjShape &shape, T center = 0,
                    BASE(T) radius = 1) {
#ifndef RELEASE
  CallStackEntry cse("Uniform");
#endif
...
template <typename T>
inline DistTensor<T> Uniform(const Grid &g, const ObjShape &shape, T center = 0,
                             BASE(T) radius = 1) {
