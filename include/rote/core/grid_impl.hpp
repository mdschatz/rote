...
inline Grid::Grid(mpi::Comm comm, const ObjShape &shape) {
#ifndef RELEASE
...
inline void Grid::SetUpGrid() {
  ...
  if (inGrid_) {
    ...
    bool reorder = false;
    mpi::CartCreate(owningComm_, order, shape.data(), periods.data(), reorder,
                    cartComm_);
  }
}
...
inline Location Grid::Loc() const {
  Location loc = gridLoc_;
  return loc;
}

inline Unsigned Grid::ModeLoc(Mode mode) const { return gridLoc_[mode]; }

inline Unsigned Grid::LinearRank() const { return linearRank_; }

inline Unsigned Grid::Order() const { return shape_.size(); }

inline Unsigned Grid::Size() const { return size_; }

inline ObjShape Grid::Shape() const { return shape_; }

inline Unsigned Grid::Dimension(Mode mode) const {
...
inline bool Grid::InGrid() const { return inGrid_; }
