...
inline Unsigned GCD(Unsigned a, Unsigned b) { return GCD_(a, b); }

inline Unsigned GCD_(Unsigned a, Unsigned b) {
  if (b == 0)
    return a;
  else
...
inline Unsigned LCM(Unsigned a, Unsigned b) { return LCM_(a, b); }

inline Unsigned LCM_(Unsigned a, Unsigned b) {
  if (a == 0 || b == 0)
    return 0;
  return a * b / (GCD(a, b));
}

inline std::vector<Unsigned> LCMs_(const std::vector<Unsigned> &a,
...
inline std::vector<Unsigned> LCMs(const std::vector<Unsigned> &a,
                                  const std::vector<Unsigned> &b) {
  return LCMs_(a, b);
}

inline Unsigned Length(Unsigned n, Unsigned shift, Unsigned wrap) {
#ifndef RELEASE
...
inline Unsigned Length_(Unsigned n, Unsigned shift, Unsigned wrap) {
  return (n > shift ? (n - shift - 1) / wrap + 1 : 0);
}

inline Unsigned Length(Unsigned n, Int rank, Unsigned alignment,
                       Unsigned wrap) {
#ifndef RELEASE
  CallStackEntry entry("Length");
#endif
...
inline Unsigned Length_(Unsigned n, Int rank, Unsigned alignment,
                        Unsigned wrap) {
  Unsigned shift = Shift_(rank, alignment, wrap);
  return Length_(n, shift, wrap);
}

inline std::vector<Unsigned> Lengths(const ObjShape &objShape,
                                     const std::vector<Unsigned> &shifts,
                                     const ObjShape &wrapShape) {
#ifndef RELEASE
  Unsigned i;
  Unsigned order;
  CallStackEntry entry("Length");
  if (!(objShape.size() == shifts.size() &&
        objShape.size() == wrapShape.size())) {
    LogicError(
        "dimensions, shifts, modulos must contain same number of elements.");
  }
  order = objShape.size();
  for (i = 0; i < order; i++) {
...
inline std::vector<Unsigned> Lengths_(const ObjShape &objShape,
                                      const std::vector<Unsigned> &shifts,
                                      const ObjShape &wrapShape) {
  Unsigned i;
...
inline Unsigned MaxLength(Unsigned n, Unsigned wrap) {
#ifndef RELEASE
...
inline Unsigned MaxLength_(Unsigned n, Unsigned wrap) {
  return (n > 0 ? (n - 1) / wrap + 1 : 0);
}

inline std::vector<Unsigned> MaxLengths(const ObjShape &shape,
                                        const ObjShape &wrapShape) {
#ifndef RELEASE
...
inline std::vector<Unsigned> MaxLengths_(const ObjShape &shape,
                                         const ObjShape &wrapShape) {
  Unsigned i;
...
inline Unsigned Shift(Int rank, Unsigned alignment, Unsigned wrap) {
#ifndef RELEASE
...
inline Unsigned Shift_(Int rank, Unsigned alignment, Unsigned wrap) {
  return (rank + wrap - alignment) % wrap;
}

// For determining the first index assigned to a given rank
inline std::vector<Unsigned> Shifts(const std::vector<Unsigned> &modeRanks,
                                    const std::vector<Unsigned> alignments,
                                    const std::vector<Unsigned> &wrapShape) {
#ifndef RELEASE
  CallStackEntry entry("Shift");
  if (modeRanks.size() != alignments.size() ||
      alignments.size() != wrapShape.size() ||
      modeRanks.size() != wrapShape.size())
...
inline std::vector<Unsigned> Shifts_(const std::vector<Unsigned> &modeRanks,
                                     const std::vector<Unsigned> alignments,
                                     const std::vector<Unsigned> &wrapShape) {
...
inline std::vector<Unsigned> Dimensions2Strides(const ObjShape &objShape) {
  std::vector<Unsigned> strides(objShape.size());
  if (strides.size() > 0) {
    strides[0] = 1;
    for (Unsigned i = 1; i < strides.size(); i++) {
      // NOTE: strides set to Max(1,c) to ensure we don't end up with 0 value
      // stride
...
inline Unsigned Loc2LinearLoc(const Location &loc, const ObjShape &shape,
                              const std::vector<Unsigned> &strides) {
  if (strides.size() != 0 &&
      (loc.size() != strides.size() || shape.size() != loc.size())) {
    //        Unsigned i;
    //        printf("loc:");
    ...
    //            printf(" %d", strides[i]);
    //        printf("\n");
    LogicError("Invalid index+stride c
ombination");
  }
...
inline Unsigned Loc2LinearLoc_(const Location &loc, const ObjShape &shape,
                               const std::vector<Unsigned> &strides) {
  Unsigned i;
...
inline Unsigned LinearLocFromStrides(const Location &loc,
                                     const std::vector<Unsigned> &strides) {
...
inline Unsigned LinearLocFromStrides_(const Location &loc,
                                      const std::vector<Unsigned> &strides) {
  Unsigned i;
...
inline Location LinearLoc2Loc(const Unsigned linearLoc, const ObjShape &shape,
...
inline Location LinearLoc2Loc_(const Unsigned linearLoc, const ObjShape &shape,
                               const Permutation &permutation) {
  Unsigned i;
...
inline Location GridViewLoc2GridLoc(const Location &gridViewLoc,
...
inline Location GridViewLoc2GridLoc_(const Location &gridViewLoc,
                                     const GridView &gridView) {
...
inline Unsigned GridViewLoc2ParticipatingLinearLoc(const Location &gridViewLoc,
                                                   const GridView &gridView) {
  return GridViewLoc2ParticipatingLinearLoc_(gridViewLoc, gridView);
}

inline Unsigned GridViewLoc2ParticipatingLinearLoc_(const Location &gridViewLoc,
                                                    const GridView &gridView) {
  ...
  for (i = 0; i < participatingOrder; i++) {
    ...
    for (j = 0; j < modeDist.size(); j++) {
      int indexOfMode = std::find(participatingComms.begin(),
                                  participatingComms.end(), modeDist[j]) -
                        participatingComms.begin();
...
inline Location GridLoc2GridViewLoc(const Location &gridLoc,
                                    const ObjShape &gridShape,
                                    const TensorDistribution &tensorDist) {
  Unsigned i;
...
inline Location
GridLoc2ParticipatingGridViewLoc(const Location &gridLoc,
                                 const ObjShape &gridShape,
                                 const TensorDistribution &tensorDist) {
  Unsigned i;
...
inline std::vector<Unsigned> IntCeils(const std::vector<Unsigned> &ms,
