...
// If the send count is one and the tag is irrelevant
template <typename T> void ISSend(T b, int to, Comm comm, Request &request);
...
template <typename T> T SendRecv(T sb, int to, int fr
om, Comm comm);
...
void Scatter(const std::complex<R> *sbuf, int sc, std::complex<R> *rbuf, int rc,
             int root, Comm comm);
// In-place option
template <typename R> void Scatter(R *buf, int sc, int rc, int root, Comm comm);
template <typename R>
...
template <typename R>
void MaxLocFunc(void *in, void *out, int *length, mpi::Datatype *datatype);
template <typename R>
void MaxLocPairFunc(void *in, void *out, int *length, mpi::Datatype *datatype);
...
template <> inline Datatype TypeMap<unsigned long long>() {
#ifdef HAVE_MPI_LONG_LONG
...
#endif // ifndef ROTE_CORE_MPI_HPP
