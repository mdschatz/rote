...
class MpiArgs {
public:
  MpiArgs(int argc, char **argv, mpi::Comm comm = mpi::COMM_WORLD,
...
inline MpiArgs::MpiArgs(int argc, char **argv, mpi::Comm comm,
                        std::ostream &error)
...
inline void MpiArgs::Process(std::ostream &os) const {
  HandleVersion(os);
  HandleBuild(os);
...
inline void MpiArgs::PrintReport(std::ostream &os) const {
  const int commRank = mpi::CommRank(comm_);
  if (commRank != 0)
    return;
...
#endif // ifndef ROTE_CORE_MPI_CHOICE_HPP
