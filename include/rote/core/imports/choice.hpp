...
template <typename TOut, typename TIn> inline TOut Cast(const TIn &input) {
...
template <> inline bool Cast(const std::string &input) {
...
public:
  ...
  template <typename T> T Input(std::string name, std::string desc);
  template <typename T>
...
protected:
  ...
  struct RequiredArg {
    ...
    RequiredArg(std::string n, std::string d, std::string t, std::string uv,
                bool f)
  ...
  struct OptionalArg {
    ...
    OptionalArg(std::string n, std::string d, std::string t, std::string dv,
                std::string uv, bool f)
        : name(n), desc(d), typeInfo(t), defaultVal(dv), usedVal(uv), found(f) {
    }
  };
...
inline Args::Args(int argc, char **argv, std::ostream &error)
    : argc_(argc), argv_(argv), usedArgs_(argc, false), error_(error) {}

template <typename T> inline T Args::Input(std::string name, std::string desc) {
  ...
  if (found) {
    ...
    if (usedArgs_[offset] || usedArgs_[offset + 1]) {
      ...
      if (usedArgs_[offset] && usedArgs_[offset + 1])
        error_ << "arguments " << offset << " and " << offset + 1 << std::endl;
      else if (usedArgs_[offset])
        error_ << "argument " << offset << std::endl;
      else
        error_ << "argument " << offset + 1 << std::endl;
      error_ << "Please ensure that you did request argument " << name
...
template <typename T>
inline T Args::Input(std::string name, std::string desc, T defaultVal) {
  char **arg = std::find(argv_, argv_ + argc_, name);
  const bool found = (arg != argv_ + argc_);
  const bool invalidFound = (arg == argv_ + argc_ - 1);
  if (invalidFound) {
...
inline void Args::Process(std::ostream &os) const {
  HandleVersion(os);
  HandleBuild(os);
...
inline void Args::PrintReport(std::ostream &os) const {
  const int numRequired = requiredArgs_.size();
  ...
  for (int i = 0; i < numRequired; ++i) {
    ...
    os << "  " << reqArg.name << " [" << reqArg.typeInfo << ","
       << reqArg.usedVal << "," << foundString << "]\n"
       << "    " << reqArg.desc << "\n\n";
  }
  ...
  for (int i = 0; i < numOptional; ++i) {
    ...
    os << "  " << optArg.name << " [" << optArg.typeInfo << ","
       << optArg.defaultVal << "," << optArg.usedVal << "," << foundString
       << "]\n"
       << "    " << optArg.desc << "\n\n";
  }

  os << "Out of " << numRequired << " required arguments, " << numReqFailed
     << " were not specified." << std::endl;

  os << "Out of " << numOptional << " optional arguments, " << numOptFailed
     << " were not specified.\n"
     << std::endl;
}
...
#endif // ifndef ROTE_CORE_CHOICE_HPP
