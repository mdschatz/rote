...
template <typename T>
inline void ViewHelper(DistTensor<T> &A, const DistTensor<T> &B,
                       bool isLocked) {
#ifndef RELEASE
  CallStackEntry entry("ViewHelper");
#endif
...
template <typename T>
inline void ViewHelper(Tensor<T> &A, const Tensor<T> &B, const Location &loc,
                       const ObjShape &shape, bool isLocked) {
#ifndef RELEASE
  CallStackEntry entry("ViewHelper");
...
template <typename T>
inline void View2x1Helper(Tensor<T> &A, const Tensor<T> &BT,
  ...
  if (AnyElemwiseNotEqual(NegFilterVector(BT.Shape(), negFilter),
                          NegFilterVector(BB.Shape(), negFilter)))
  ...
  if (BB.LockedBuffer() !=
      (BT.LockedBuffer() + BT.Dimension(mode) * BT.Stride(mode)))
...
template <typename T>
inline void View2x1Helper(DistTensor<T> &A, const DistTensor<T> &BT,
  ...
  if (A.Participating()) {
    ...
    View2x1Helper(A.Tensor(), BT.LockedTensor(), BB.LockedTensor(), mode,
                  isLocked);
  } else {
    std::fill(A.modeShifts_.begin(), A.modeShifts_.end(), 0);
  }
...
template <typename T>
inline void ViewAsLowerOrderHelper(Tensor<T> &A, const Tensor<T> &B,
  ...
  for (i = 1; i < newOrder; i++) {
    ...
    // NOTE: strides are set to Max(c, 1) to ensure we don't end up with 0 value
    // strides
...
template <typename T>
inline void ViewAsHigherOrderHelper(Tensor<T> &A,
  ...
  for (i = 0; i < splitShape.size(); i++) {
    ...
    for (j = 0; j < newModeGroupS
hape.size(); j++) {
      A.shape_[modeCount] = newModeGroupShape[j];
      if (modeCount == 0) {
        A.strides_[modeCount] = 1;
      } else {
        A.strides_[modeCount] =
            A.shape_[modeCount - 1] * A.strides_[modeCount - 1];
      }
      modeCount++;
    }
  }
...
template <typename T>
inline void ViewAsMatrixHelper(Tensor<T> &A, const Tensor<T> &B,
  ...
  if (nModesMergeCol > 0 && order - nModesMergeCol > 0) {
    ...
    if (order == 0) {
      ...
      if (mergeModes[0].size() == 0) {
        ...
        A.shape_[1] = prod(FilterVector(shapeB, mergeModes[1]));
        A.strides_[1] =
            Max(1, stridesB[nModesMergeCol - 1] * shapeB[nModesMergeCol - 1]);
      }
    }
    if (isLocked)
      A.viewType_ = LOCKED_VIEW;
...
template <typename T> inline void View(Tensor<T> &A, Tensor<T> &B) {
#ifndef RELEASE
...
template <typename T> inline Tensor<T> View(Tensor<T> &B) {
  Tensor<T> A(B.Order());
  View(A, B);
  return A;
}

template <typename T> inline void View(DistTensor<T> &A, DistTensor<T> &B) {
#ifndef RELEASE
  CallStackEntry entry("View");
#endif
  ViewHelper(A, B, false);
...
template <typename T> inline DistTensor<T> View(DistTensor<T> &B) {
  DistTensor<T> A(B.Order(), B.Grid());
  View(A, B);
  return A;
}

template <typename T> inline void LockedView(Tensor<T> &A, const Tensor<T> &B) {
#ifndef RELEASE
...
template <typename T> inline Tensor<T> LockedView(const Tensor<T> &B) {
  Tensor<T> A(B.Order());
  LockedView(A, B);
  return A;
}
...
template <typename T> inline DistTensor<T> LockedView(const DistTensor<T> &B) {
  DistTensor<T> A(B.Order(), B.Grid());
  LockedView(A, B);
  return A;
}
...
template <typename T>
inline void View(Tensor<T> &A, Tensor<T> &B, const Location &loc,
...
template <typename T>
inline Tensor<T> View(Tensor<T> &B, const Location &loc,
                      const ObjShape &shape) {
  Tensor<T> A(B.Order());
  View(A, B, loc, shape);
  return A;
...
template <typename T>
inline void View(DistTensor<T> &A, DistTensor<T> &B, const Location &loc,
                 const ObjShape &shape) {
#ifndef RELEASE
  CallStackEntry entry("View");
#endif
  ...
  if (A.Participating()) {
    const std::vector<Unsigned> modeWrapStrides = B.GridViewShape();
    const std::vector<Unsigned> localShapeBehind =
        Lengths(loc, B.ModeShifts(), modeWrapStrides);
    const std::vector<Unsigned> localShape =
        Lengths(shape, A.ModeShifts(), modeWrapStrides);
...
template <typename T>
inline DistTensor<T> View(DistTensor<T> &B, const Location &loc,
                          const ObjShape &shape) {
  DistTensor<T> A(B.Order(), B.Grid());
  View(A, B, loc, shape);
  return A;
...
template <typename T>
inline void LockedView(Tensor<T> &A, const Tensor<T> &B, const Location &loc,
                       const ObjShape &shape) {
#ifndef RELEASE
  CallStackEntry entry("LockedView");
#endif
...
template <typename T>
inline Tensor<T> LockedView(const Tensor<T> &B, const Location &loc,
                            const ObjShape &shape) {
  Tensor<T> A(B.Order());
...
template <typename T>
inline DistTensor<T> LockedView(const DistTensor<T> &B, const Location &loc,
                                const ObjShape &shape) {
  DistTensor<T> A(B.Order(), B.Grid());
  LockedView(A, B, loc, shape);
  return A;
...
template <typename T>
inline void View2x1(DistTensor<T> &A, DistTensor<T> &BT, DistTensor<T> &BB,
                    Mode mode) {
#ifndef RELEASE
  CallStackEntry entry("View2x1");
#endif
...
template <typename T>
inline void LockedView2x1(Tensor<T> &A, const Tensor<T> &BT,
...
template <typename T>
inline Tensor<T> LockedView2x1(const Tensor<T> &BT, const Tensor<T> &BB,
                               Mode mode) {
  Tensor<T> A(BT.Order());
...
template <typename T>
inline void LockedView2x1(DistTensor<T> &A, const DistTensor<T> &BT,
...
template <typename T>
inline DistTensor<T> LockedView2x1(const DistTensor<T> &BT,
                                   const DistTensor<T> &BB, Mode mode) {
  DistTensor<T> A(BT.Order(), BT.Grid());
  LockedView2x1(A, BT, BB, mode);
  return A;
...
template <typename T>
inline void ViewAsLowerOrder(Tensor<T> &A, Tensor<T> &B,
                             const std::vector<ModeArray> &oldModes) {
#ifndef RELEASE
...
template <typename T>
inline Tensor<T> ViewAsLowerOrder(Tensor<T> &B,
                                  const std::vector<ModeArray> &oldModes) {
  Tensor<T> A(B.Order());
...
template <typename T>
inline void LockedViewAsLowerOrder(Tensor<T> &A, const Tensor<T> &B,
                                   const std::vector<ModeArray> &oldModes) {
#ifndef RELEASE
...
template <typename T>
inline Tensor<T>
LockedViewAsLowerOrder(const Tensor<T> &B,
                       const std::vector<ModeArray> &oldModes) {
  Tensor<T> A(B.Order());
...
template <typename T>
inline void ViewAsHigherOrder(Tensor<T> &A, Tensor<T> &B,
                              const std::vector<ObjShape> &splitShape) {
#ifndef RELEASE
...
template <typename T>
inline Tensor<T> ViewAsHigherOrder(Tensor<T> &B,
                                   const std::vector<ObjShape> &splitShape) {
  Tensor<T> A(B.Order());
...
template <typename T>
inline void LockedViewAsHigherOrder(Tensor<T> &A, const Tensor<T> &B,
                                    const std::vector<ObjShape> &splitShape) {
#ifndef RELEASE
  CallStackEntry entry("ViewAsHigherOrder");
#endif
...
template <typename T>
inline Tensor<T>
LockedViewAsHigherOrder(const Tensor<T> &B,
                        const std::vector<ObjShape> &splitShape) {
  Tensor<T> A(B.Order());
...
template <typename T>
inline void ViewAsMatrix(Tensor<T> &A, const Tensor<T> &B,
...
template <typename T>
inline Tensor<T> ViewAsMatrix(const Tensor<T> &B,
                              const Unsigned &nModesMergeCol) {
  Tensor<T> A(2);
...
template <typename T>
inline void LockedViewAsMatrix(Tensor<T> &A, const Tensor<T> &B,
                               const Unsigned &nModesMergeCol) {
#ifndef RELEASE
  CallStackEntry entry("LockedViewAsLowerOrder");
#endif
...
template <typename T>
inline Tensor<T> LockedViewAsMatrix(const Tensor<T> &B,
                                    const Unsigned &nModesMergeCol) {
  Tensor<T> A(2);
