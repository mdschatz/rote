...
template <typename Real> struct Base { typedef Real type; };
template <typename Real> struct Base<std::complex<Real>> { typedef Real type; };
...
template <typename Real> struct ValueInt<std::complex<Real>> {
...
template <typename Real> struct ValueIntPair {
  Real value;
  Int indices[2];
...
template <typename Real> struct ValueIntPair<std::complex<Real>> {
  std::complex<Real> value;
  Int indices[2];

  static bool Lesser(const ValueIntPair<Real> &a, const ValueIntPair<Real> &b) {
    return Abs(a.value) < Abs(b.value);
  }
  static bool Greater(const ValueIntPair<Real> &a,
                      const ValueIntPair<Real> &b) {
    return Abs(a.value) > Abs(b.value);
  }
};
...
static inline bool IsOwner(ViewType v) { return (v & VIEW) == 0; }
static inline bool IsViewing(ViewType v) { return (v & VIEW) != 0; }
static inline bool IsShrinkable(ViewType v) { return (v & OWNER_FIXED) == 0; }
static inline bool IsFixedSize(ViewType v) { return (v & OWNER_FIXED) != 0; }
static inline bool IsUnlocked(ViewType v) { return (v & LOCKED_OWNER) == 0; }
static inline bool IsLocked(ViewType v) { return (v & LOCKED_OWNER) != 0; }
} // namespace viewtype_wrapper
