...
template <typename T> T Input(std::string name, std::string desc, T defaultVal);
void ProcessInput();
...
inline double Ceil(double m) { return std::ceil(m); }

inline Int Max(Int m, Int n) { return std::max(m, n); }

inline Int Min(Int m, Int n) { return std::min(m, n); }
...
template <typename T> void MemSwap(T *a, T *b, T *temp, std::size_t numEntries);

// Generalization of std::memcpy so that unit strides are not required
template <typename T>
void StridedMemCopy(T *dest, std::size_t destStride, const T *source,
...
template <typename T> void MemZero(T *buffer, std::size_t numEntries);
