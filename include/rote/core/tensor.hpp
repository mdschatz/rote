...
public:
  ...
  void AssertValidDimensions(const ObjShape &shape,
                             const std::vector<Unsigned> &strides) const;
  ...
  Tensor(const ObjShape &shape, const std::vector<Unsigned> &strides,
         bool fixed = false);
  Tensor(const ObjShape &shape, const T *buffer,
         const std::vector<Unsigned> &strides, bool fixed = false);
  Tensor(const ObjShape &shape, T *buffer, const std::vector<Unsigned> &strides,
         bool fixed = false);
  ...
  void Attach(const ObjShape &shape, T *buffer,
              const std::vector<Unsigned> &strides);
  void LockedAttach(const ObjShape &shape, const T *buffer,
                    const std::vector<Unsigned> &strides);
  ...
  void CopyBuffer(const Tensor &A, const Permutation &srcPerm,
...
private:
  ...
  void Control_(const ObjShape &shape, T *buffer,
                const std::vector<Unsigned> &strides);
  void Attach_(const ObjShape &shape, T *buffer,
               const std::vector<Unsigned> &strides);
  void LockedAttach_(const ObjShape &shape, const T *buffer,
                     const std::vector<Unsigned> &strides);

  template <typename F> friend class Tensor;
  template <typename F> friend class DistTensor;
  friend class DistTensorBase<T>;

  friend void ViewHelper<T>(Tensor<T> &A, const Tensor<T> &B, bool isLocked);
  friend void ViewHelper<T>(Tensor<T> &A, const Tensor<T> &B,
                            const Location &loc, const ObjShape &shape,
                            bool isLocked);
  friend void View2x1Helper<T>(Tensor<T> &A, const Tensor<T> &BT,
  ...
  friend void ViewAsLowerOrderHelper<T>(Tensor<T> &A, const Tensor<T> &B,
                                        ...
                                        bool isLocked);
  friend void ViewAsHigherOrderHelper<T>(
      Tensor<T> &A, const std::vector<ObjShape> &splitShape, bool isLocked);
  ...
  friend void View<T>(Tensor<T> &A, Tensor<T> &B, const Location &loc,
                      const ObjShape &shape);
  friend void LockedView<T>(Tensor<T> &A, const Tensor<T> &B,
                            const Location &loc, const ObjShape &shape);

  friend void View2x1<T>(Tensor<T> &A, Tensor<T> &BT, Tensor<T> &BB, Mode mode);
  friend void LockedView2x1<T>(Tensor<T> &A, const Tensor<T> &BT,
                               const Tensor<T> &BB, Mode mode);

  friend void ViewAsLowerOrder<T>(Tensor<T> &A, Tensor<T> &B,
                                  const std::vector<ModeArray> &oldModes);
  friend void LockedViewAsLowerOrder<T>(Tensor<T> &A, const Tensor<T> &B,
                                        const std::vector<ModeArray> &oldModes);

  friend void ViewAsHigherOrder<T>(Tensor<T> &A, Tensor<T> &B,
                                   const std::vector<ObjShape> &splitShape);
  friend void
  LockedViewAsHigherOrder<T>(Tensor<T> &A, const Tensor<T> &B,
                             const std::vector<ObjShape> &splitShape);

  friend void ViewAsMatrix<T>(Tensor<T> &A, const Tensor<T> &B,
                              const Unsigned &nModesMergeCol);
  friend void LockedViewAsMatrix<T>(Tensor<T> &A, const Tensor<T> &B,
                                    const Unsigned &nModesMergeCol);
};
