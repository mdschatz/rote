...
*/
#pragma once
#ifndef ROTE_CORE_RANDOM_DECL_HPP
#define ROTE_CORE_RANDOM_DECL_HPP
...
template <typename T> T UnitCell();

template <typename T> T Uniform(T a = 0, T b = UnitCell<T>());
...
template <typename T> T Normal(T mean = 0, BASE(T) stddev = 1);
...
template <typename T> T SampleBall(T center = 0, BASE(T) radius = 1);

} // namespace rote

#endif // ifndef ROTE_CORE_RANDOM_DECL_HPP
