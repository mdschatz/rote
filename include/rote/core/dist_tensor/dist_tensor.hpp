...
public:
  ...
  void AssertValidSubtensor(const Location &loc, const ObjShape &shape) const;
  ...
  DistTensorBase(const TensorDistribution &dist,
                 const rote::Grid &g = DefaultGrid());

  // Create a "shape" distributed tensor
  DistTensorBase(const ObjShape &shape, const TensorDistribution &dist,
                 const rote::Grid &g = DefaultGrid());
  ...
  // Create a "shape" distributed tensor
  DistTensorBase(const ObjShape &shape, const std::string &dist,
                 const rote::Grid &g = DefaultGrid());
...
// DistTensor<T>& operator=( DistTensor<T>&& A );
  ...
  Location
  DetermineOwnerNewAlignment(const Location &loc,
                             std::vector<Unsigned> &newAlignment) const;
  Location DetermineFirstElem(const Location &gridLoc) const;
  Location
  DetermineFirstUnalignedElem(const Location &gridViewLoc,
  ...
  ModeArray GetMisalignedModes(const DistTensor<T> &B) const;
...
T alpha);
  ...
  void AlignModesWith(const ModeArray &modes, const DistTensorBase<T> &A,
  ...
  void Attach(const ObjShape &shape, const std::vector<Unsigned> &modeAligns,
              T *buffer, const std::vector<Unsigned> &strides,
              const rote::Grid &grid);
  void LockedAttach(const ObjShape &shape,
                    const std::vector<Unsigned> &modeAligns, const T *buffer,
  ...
  void GetRealPartOfDiagonal(DistTensor<BASE(T)> &d, Int offset = 0) const;
  void GetImagPartOfDiagonal(DistTensor<BASE(T)> &d, Int offset = 0) const;
  DistTensor<BASE(T)> GetRealPartOfDiagonal(Int offset = 0) const;
  DistTensor<BASE(T)> GetImagPartOfDiagonal(Int offset = 0) const;

  void SetRealPartOfDiagonal(const DistTensor<BASE(T)> &d, Int offset = 0);
  // Only valid for complex datatypes
  void SetImagPartOfDiagonal(const DistTensor<BASE(T)> &d, Int offset = 0);
...
private:
  ...
  void SetAlignmentsAndResize(const std::vector<Unsigned> &aligns,
                              const ObjShape &shape);
  void ForceAlignmentsAndResize(const std::vector<Unsigned> &aligns,
                                const ObjShape &shape);

  void SetModeAlignmentAndResize(Mode mode, Unsigned align,
                                 const ObjShape &shape);
  void ForceModeAlignmentAndResize(Mode mode, Unsigned align,
                                   const ObjShape &shape);

#ifndef SWIG
  template <typename S> friend class DistTensor;

  template <typename S>
  friend void ViewHelper(DistTensor<S> &A, const DistTensor<S> &B,
                         bool isLocked);
  template <typename S>
  friend void ViewHelper(DistTensor<S> &A, const DistTensor<S> &B,
                         ...
                         bool isLocked);
  template <typename S>
  friend void View2x1Helper(DistTensor<S> &A, const DistTensor<S> &BT,
                            const DistTensor<S> &BB, Mode mode, bool isLocked);
