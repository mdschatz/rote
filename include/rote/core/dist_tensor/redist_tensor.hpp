...
#ifndef RELEASE
template <typename T>
void AssertConforming2x1(const DistTensor<T> &AT, const DistTensor<T> &AB,
                         Mode mode);
#endif

template <typename T> class DistTensor : public DistTensorBase<T> {
public:
  ...
  DistTensor(const TensorDistribution &dist,
             const rote::Grid &g = DefaultGrid());

  // Create a "shape" distributed tensor
  DistTensor(const ObjShape &shape, const TensorDistribution &dist,
             const rote::Grid &g = DefaultGrid());

  // Create a "shape" distributed tensor with specified alignments
  DistTensor(const ObjShape &shape, const TensorDistribution &dist,
             const std::vector<Unsigned> &modeAligns, const rote::Grid &g);

  // Create a "shape" distributed tensor with specified alignments
  // and leading dimension
  DistTensor(const ObjShape &shape, const TensorDistribution &dist,
             const std::vector<Unsigned> &modeAligns,
             const std::vector<Unsigned> &strides, const rote::Grid &g);

  // View a constant distributed tensor's buffer
  DistTensor(const ObjShape &shape, const TensorDistribution &dist,
             const std::vector<Unsigned> &modeAligns, const T *buffer,
             const std::vector<Unsigned> &strides, const rote::Grid &g);

  // View a mutable distributed tensor's buffer
  DistTensor(const ObjShape &shape, const TensorDistribution &dist,
             const std::vector<Unsigned> &modeAligns, T *buffer,
             const std::vector<Unsigned> &strides, const rote::Grid &g);
  ...
  // Create a "shape" distributed tensor
  DistTensor(const ObjShape &shape, const std::string &dist,
             const rote::Grid &g = DefaultGrid());
  ...
  //
  void PermutationRedistFrom(const DistTensor<T> &A,
  ...
  //
  void ReduceUpdateRedistFrom(const RedistType &redistType, const T alpha,
                              const DistTensor<T> &A, const T beta,
                              const ModeArray &reduceModes);

  //
  // AllReduce interface routines
  //
  void AllReduceUpdateRedistFrom(const T alpha, const DistTensor<T> &A,
                                 const T beta, const ModeArray &reduceModes);
  void AllReduceRedistFrom(const DistTensor<T> &A, const Mode reduceMode);
  void AllReduceUpdateRedistFrom(const DistTensor<T> &A, const T beta,
                                 const Mode reduceMode);
  void AllReduceRedistFrom(const DistTensor<T> &A,
                           const ModeArray &reduceModes);
  void AllReduceUpdateRedistFrom(const DistTensor<T> &A, const T beta,
                                 const ModeArray &reduceModes);
  ...
  //
  void ReduceScatterUpdateRedistFrom(const T alpha, const DistTensor<T> &A,
                                     const T beta,
                                     const ModeArray &reduceModes);
  void ReduceScatterRedistFrom(const DistTensor<T> &A, const Mode reduceMode);
  void ReduceScatterRedistFrom(const T alpha, const DistTensor<T> &A,
                               const Mode reduceMode);
  void ReduceScatterUpdateRedistFrom(const DistTensor<T> &A, const T beta,
                                     const Mode reduceMode);
  void ReduceScatterRedistFrom(const T alpha, const DistTensor<T> &A,
                               const ModeArray &reduceModes);
  void ReduceScatterRedistFrom(const DistTensor<T> &A,
                               const ModeArray &reduceModes);
  void ReduceScatterUpdateRedistFrom(const DistTensor<T> &A, const T beta,
                                     const ModeArray &reduceModes);
  void ReduceScatterUpdateRedistFrom(const DistTensor<T> &A,
                                     const ModeArray &reduceModes);

  //
  // Reduce-to-one interface routines
  //
  void ReduceToOneUpdateRedistFrom(T alpha, const DistTensor<T> &A,
                                   const T beta, const ModeArray &rModes);
  ...
  void ReduceToOneUpdateRedistFrom(const DistTensor<T> &A, const T beta,
...
private:
  ...
  void Comm
Redist(const TensorDistribution &finalDist,
                  const TensorDistribution &startingDist,
                  RedistPlanInfo &redistData, std::vector<Redist> &intDists);
  RedistPlanInfo
  CreateGenRedistData(const TensorDistribution &startDist,
                      const TensorDistribution &endDist,
                      const ModeArray &reduceModes = ModeArray());
  void CommRedistReduce(const TensorDistribution &finalDist,
                        const TensorDistribution &startingDist,
                        RedistPlanInfo &redistData,
                        std::vector<Redist> &intDists);
  void CommRedistAdd(const TensorDistribution &finalDist,
                     const TensorDistribution &startingDist,
                     RedistPlanInfo &redistData, std::vector<Redist> &intDists);
  void CommRedistMove(const TensorDistribution &finalDist,
                      const TensorDistribution &startingDist,
                      RedistPlanInfo &redistData,
                      std::vector<Redist> &intDists);
  void CommRedistP2P(const TensorDistribution &finalDist,
                     const TensorDistribution &startingDist,
                     RedistPlanInfo &redistData, std::vector<Redist> &intDists);
  void CommRedistRemove(const TensorDistribution &finalDist,
                        const TensorDistribution &startingDist,
                        RedistPlanInfo &redistData,
                        std::vector<Redist> &intDists);
  ...
  void AllToAllCommRedist(const DistTensor<T> &A, const ModeArray &commModes,
                          const T alpha = T(0));
  void PackA2ACommSendBuf(const DistTensor<T> &A, const ModeArray &commModes,
                          const ObjShape &sendShape, T *const sendBuf);
  void UnpackA2ACommRecvBuf(const T *const recvBuf, const ModeArray &commModes,
                            const ObjShape &sendShape, const DistTensor<T> &A,
                            const T alpha = T(0));
  ...
  void PermutationCommRedist(const DistTensor<T> &A,
                             const ModeArray &redistModes,
                             const T alpha = T(0));
  ...
  void AllReduceUpdateCommRedist(const T alpha, const DistTensor<T> &A,
                                 const T beta, const ModeArray &commModes);
  void PackARCommSendBuf(const DistTensor<T> &A, const ModeArray &reduceModes,
                         const ModeArray &commModes, T *const sendBuf);
  void UnpackARUCommRecvBuf(const T *const recvBuf, const T alpha,
                            const DistTensor<T> &A, const T beta);
  ...
  void ReduceScatterUpdateCommRedist(const T alpha, const DistTensor<T> &A,
                                     const T beta, const ModeArray &reduceModes,
                                     const ModeArray &commModes);
  void PackRSCommSendBuf(const DistTensor<T> &A, const ModeArray &reduceModes,
                         const ModeArray &commModes, T *const sendBuf);
  void UnpackRSUCommRecvBuf(const T *const recvBuf, const T alpha,
                            const T beta);
  ...
  void ReduceToOneUpdateCommRedist(const T alpha, const DistTensor<T> &A,
                                   const T beta, const ModeArray &commModes);
  ...
  bool AlignCommBufRedist(const DistTensor<T> &A, const T *unalignedSendBuf,
                          const Unsigned sendSize, T *alignedSendBuf,
                          const Unsigned recvSize);
};

} // namespace rote

#endif // ifndef ROTE_CORE_DISTTENSOR_DECL_HPP
