...
template <typename T>
inline void ReadBinarySeqPack(const DistTensor<T> &A, const ObjShape &packShape,
                              const ObjShape &commGridViewShape,
                              const Unsigned &nElemsPerProc,
                              const Location &fileLoc,
                              const ObjShape &gblDataShape,
                              std::ifstream &fileStream, T *sendBuf) {
  ...
  // Info about how much data we have packed that we possibly can pack (memory
  // constraint)
  ...
  while (!done) {
    ...
    while (packedPtr < packShape.size() &&
           packedShape[packedPtr] >= packShape[packedPtr]) {
    ...
    // Update the counters
    while (gblDataPtr < order &&
           gblDataLoc[gblDataPtr] >= gblDataShape[gblDataPtr]) {
...
template <typename T>
inline void ReadAsciiSeqPack(const DistTensor<T> &A, const ObjShape &packShape,
                             const ObjShape &commGridViewShape,
                             const Unsigned &nElemsPerProc,
                             const Location &fileLoc,
                             const ObjShape &gblDataShape,
                             std::stringstream &dataStream, T *sendBuf) {
  Unsigned order = packShape.size();
...
template <typename T>
inline void ReadSeqUnpack(DistTensor<T> &A, const Location &firstGblLoc,
                          const ObjShape &recvShape, const T *recvBuf) {
  ...
  bool done = !(ElemwiseLessThan(localUnpackLoc, localShape) &&
                ElemwiseLessThan(recvLoc, recvShape));
  ...
  while (!done) {
    ...
    while (localUnpackPtr < order &&
           localUnpackLoc[localUnpackPtr] >= localShape[localUnpackPtr]) {
...
template <typename T>
void ReadSeq(DistTensor<T> &A, const std::string filename, FileFormat format) {
  std::ifstream file;
  switch (format) {
  case ASCII_MATLAB:
  case ASCII:
...
.open(filename.c_str());
  ...
  Unsigned remainder =
      MaxLength(MAX_ELEM_PER_PROC, prod(gvA.ParticipatingShape()));
  ...
  while (!done) {
    if (A.Grid().LinearRank() == 0) {
      ...
      if (format == ASCII || format == ASCII_MATLAB) {
        ReadAsciiSeqPack(A, packetShape, gvAShape, prod(sendShape), dataLoc,
                         dataShape, dataStream, &(sendBuf[0]));
      } else {
        ReadBinarySeqPack(A, packetShape, gvAShape, prod(sendShape), dataLoc,
                          dataShape, file, &(sendBuf[0]));
      }
    ...
    myFirstGblElemLocUnpack =
        ElemwiseSum(dataLoc, ElemwiseSubtract(myPackGridViewLoc, owner));

    if (ElemwiseLessThan(myFirstGblElemLocUnpack, dataShape) &&
        ElemwiseLessThanEqualTo(dataLoc, myFirstGblElemLocUnpack) &&
        !AnyElemwiseGreaterThan(myFirstGblElemLocUnpack, packLastLoc)) {
...
template <typename T>
void ReadNonSeq(DistTensor<T> &A, const std::string filename,
                FileFormat format) {
  std::ifstream file;
  switch (format) {
  case ASCII_MATLAB:
...
template <typename T>
void Read(DistTensor<T> &A, const std::string filename, FileFormat format,
          bool sequential) {
  if (format == AUTO)
    format = DetectFormat(filename);
...
#define FULL(T)                                                                \
  template void Read(Tensor<T> &A, const std::string filename,                 \
                     FileFormat format);                                       \
  template void Read(DistTensor<T> &A, const std::string filename,             \
