...
double minRealWindowVal, maxRealWindowVal, minImagWindowVal, maxImagWindowVal;
#endif
...
void PrintVersion(std::ostream &os) {
  os << "ROTE version information:\n"
     ...
     << "  Version:      " << ROTE_VERSION_MAJOR << "." << ROTE_VERSION_MINOR
     << "\n"
...
bool Initialized() { return ::numElemInits > 0; }
...
void Initialize(int &argc, char **&argv) {
  ...
  if (!mpi::Initialized()) {
    if (mpi::Finalized()) {
      LogicError("Cannot initialize rote after finalizing MPI");
    }
#ifdef HAVE_OPENMP
    const Int provided =
        mpi::InitializeThread(argc, argv, mpi::THREAD_MULTIPLE);
...
Int Blocksize() { return ::blocksizeStack.top(); }

void SetBlocksize(Int blocksize) { ::blocksizeStack.top() = blocksize; }

void PushBlocksizeStack(Int blocksize) { ::blocksizeStack.push(blocksize); }

void PopBlocksizeStack() { ::blocksizeStack.pop(); }
