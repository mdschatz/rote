...
template <typename T>
RedistPlanInfo
DistTensor<T>::CreateGenRedistData(const TensorDistribution &tenDistA,
                                   const TensorDistribution &tenDistB,
  ...
  redistData.gridModesRemovedSrcs =
      GetModeDistOfGridMode(redistData.gridModesRemoved, tenDistA);
  ...
  redistData.gridModesAppearedSinks =
      GetModeDistOfGridMode(redistData.gridModesAppeared, tenDistB);

  // Determine grid modes that moved mode distributions
  // Determine their Src tensor mode dist along the way
  // Note: Adjusting for any reductions that may occur (we assume reductions are
  ...
  TensorDistribution srcTenDist =
      GetTensorDistForGridModes(tenDistA, nonReducedGridModes);
  srcTenDist.RemoveUnitModeDists(reduceModes);
  TensorDistribution sinkTenDist =
      GetTensorDistForGridModes(tenDistB, nonReducedGridModes);
...
void DistTensor<T>::RedistFrom(const DistTensor<T> &A,
                               const ModeArray &reduceModes, const T alpha,
                               const T beta) {
  ...
  RedistPlanInfo redistData =
      CreateGenRedistData(A.TensorDist(), this->TensorDist(), reduceModes);
  ...
  tmp.LockedAttach(A.Shape(), A.Alignments(), A.LockedBuffer(),
                   A.LocalPermutation(), A.LocalStrides(), g);
  ...
  for (i = 0; i < intermediateDists.size() - 1; i++) {
    ...
    case AG:
      tmp2.AllGatherRedistFrom(tmp, intRedist.modes);
      break;
    case A2A:
      tmp2.AllToAllRedistFrom(tmp, intRedist.modes);
      break;
    case Perm:
      tmp2.PermutationRedistFrom(tmp, intRedist.modes);
      break;
    case Local:
      tmp2.LocalRedistFrom(tmp);
      break;
    //    	case AR: tmp2.AllReduceRedistFrom(tmp, reduceModes); break;
    case RS:
      tmp2.ReduceScatterRedistFrom(alpha, tmp, reduceModes);
      break;
    default:
      break;
    }
  ...
  case AG:
    AllGatherRedistFrom(tmp, lastRedist.modes, beta);
    break;
  case A2A:
    AllToAllRedistFrom(tmp, lastRedist.modes, beta);
    break;
  case Local:
    LocalRedistFrom(tmp);
    break;
  case Perm:
    PermutationRedistFrom(tmp, lastRedist.modes, beta);
    break;
  //	case AR: tmp2.AllReduceUpdateRedistFrom(tmp, beta, reduceModes); break;
  case RS:
    ReduceScatterUpdateRedistFrom(tmp, beta, reduceModes);
    break;
  default:
    break;
  }
...
template <typename T> void DistTensor<T>::RedistFrom(const DistTensor<T> &A) {
