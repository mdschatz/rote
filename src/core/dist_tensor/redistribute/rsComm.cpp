...
template <typename T>
void DistTensor<T>::ReduceScatterUpdateCommRedist(const T alpha,
...
template <typename T>
void DistTensor<T>::PackRSCommSendBuf(const DistTensor<T> &A,
  ...
  for (Unsigned i = 0; i < nRedistProcsAll; i++) {
    ...
    // Note the following 4 work, but accidentally assign a valid firstLoc no
    // matter what.
    //        Location myFirstLoc =
    //        A.DetermineFirstElem(A.GetGridView().ParticipatingLoc()); Location
    //        myFirstLocUnaligned =
    //        ElemwiseMod(ElemwiseSum(ElemwiseSubtract(myFirstLoc,
    //        A.Alignments()), A.Shape()), A.Shape()); Location procFirstLoc =
    //        DetermineFirstElem(GridLoc2ParticipatingGridViewLoc(procGridLoc,
    //        gridShape, TensorDist())); Location procFirstLocUnaligned =
    //        ElemwiseMod(ElemwiseSum(ElemwiseSubtract(procFirstLoc,
    //        Alignments()), Shape()), Shape());

    // Get my first elem location
    Location myFirstLoc =
        A.DetermineFirstElem(A.GetGridView().ParticipatingLoc());

    // Determine what grid location p_i corresponds to AFTER alignment has been
    // performed  so we correctly determine what elements to pack
    //        Location firstElemOwnerA = GridViewLoc2GridLoc(A.Alignments(),
    //        gvA); Location firstElemOwnerB = GridViewLoc2GridLoc(Alignments(),
    //        gvB); Location alignDiff = ElemwiseSubtract(firstElemOwnerB,
    //        firstElemOwnerA); Location procLocAfterRealign =
    //        ElemwiseMod(ElemwiseSum(ElemwiseSum(procGridLoc, alignDiff),
    //        g.Shape()), g.Shape());
    ...
    for (j = 0; j < nonRModes.size(); j++) {
      ...
      //            Unsigned sendFirstIndex =
      //            adjustedProcFirstElemLoc[nonRMode];
      Unsigned myModeStride = A.ModeStride(nonRMode);
      Unsigned sendProcModeStride = this->ModeStride(nonRMode);

      while (myFirstIndex != sen
dFirstIndex &&
             myFirstIndex < this->Dimension(nonRMode)) {
    ...
    //        Location actualFirstSendLoc =
    //        ElemwiseMod(ElemwiseSum(firstSendLoc, A.Alignments()), A.Shape());
    ...
    if (found && ElemwiseLessThan(firstRecvLoc, this->Shape()) &&
        ElemwiseLessThan(firstSendLoc, A.Shape())) {
      ...
      PackCommHelper(packData, &(dataBuf[dataBufPtr]),
...
template <typename T>
void DistTensor<T>::UnpackRSUCommRecvBuf(const T *const recvBuf, const T alpha,
                                         const T beta) {
  ...
  data.srcStrides = Dimensions2Strides(
