...
// TODO: Check that logic works out (modeStrides being global info applied to
// local info)
template <typename T>
void DistTensor<T>::UnpackLocalCommRecvBuf(const DistTensor<T> &A,
                                           const T *recvBuf, const T alpha) {

  const Location myFirstLocB =
      this->DetermineFirstElem(this->GetGridView().ParticipatingLoc());
  const Location myFirstLocA =
  ...
  Location unpackProcGVA =
      GridLoc2ParticipatingGridViewLoc(myGridLoc, g.Shape(), A.TensorDist());
  ...
  Unsigned dataBufPtr = LinearLocFromStrides(
      ElemwiseDivide(ElemwiseSubtract(myFirstLocB, myFirstElemLocAligned),
                     gvAShape),
      Dimensions2Strides(A.MaxLocalShape()));
  ...
  unpackData.srcBufStrides =
      ElemwiseProd(Dimensions2Strides(A.MaxLocalShape()), modeStrideFactor);
