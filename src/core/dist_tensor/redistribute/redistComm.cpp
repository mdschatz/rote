...
template <typename T>
void DistTensor<T>::CommRedistReduce(const TensorDistribution &finalDist,
                                     const TensorDistribution &startDist,
  ...
  ModeDistribution gridModesReducedAndDisappeared =
      gridModesReduced - finalDist.UsedModes();
  ModeDistribution gridModesReducedAndKept =
      gridModesReduced - gridModesReducedAndDisappeared;
  TensorDistribution finalReduceModesDist =
      GetTensorDistForGridModes(finalDist, gridModesReducedAndKept);
  std::vector<ModeDistribution> tenDistReducedAndDisappearedVals(
      intDist.size());
  TensorDistribution tenDistReducedAndDisappeared(
      tenDistReducedAndDisappearedVals);
...
template <typename T>
void DistTensor<T>::CommRedistAdd(const TensorDistribution &finalDist,
                                  const TensorDistribution &startDist,
                                  RedistPlanInfo &redistData,
                                  std::vector<Redist> &intDists) {
  // Get the prefix tensor distribution
  TensorDistribution intDist = startDist;
  TensorDistribution appearedTenDist =
      GetTensorDistForGridModes(finalDist, redistData.gridModesAppeared);
...
template <typename T>
void DistTensor<T>::CommRedistRemove(const TensorDistribution &finalDist,
                                     const TensorDistribution &startDist,
                                     RedistPlanInfo &redistData,
                                     std::vector<Redist> &intDists) {
  // Get the prefix tensor distribution
  TensorDistribution intDist = startDist;
  TensorDistribution removedModesTenDist =
      GetTensorDistForGridModes(startDist, redistData.gridModesRemoved);
...
template <typename T>
void DistTensor<T>::CommRedistMove(const TensorDistribution &finalDist,
                                   const TensorDistribution &startDist,
                                   RedistPlanInfo &redistData,
                                   std::vector<Redist> &intDists) {
  Unsigned i;
  ...
  while (possModes.size() > 0) {
    ...
    if (!modesToMoveSrcs.Contains(possModeSink) &&
        !modesToMoveSinks.Contains(possModeSrc)) {
  ...
  for (i = 0; i < possModes.size(); i++) {
    // std::cout << "attempting to remove gMode: " << modesToMove[i] <<
    // std::endl; std::cout << "attempting to remove gModeSrc: " <<
    // modesToMoveSrcs[i] << std::endl; std::cout << "attempting to remove
    // gModeSink: " << modesToMoveSinks[i] << std::endl;
    redistData.gridModesMoved.erase(std::find(redistData.gridModesMoved.begin(),
                                              redistData.gridModesMoved.end(),
                                              modesToMove[i]));
    redistData.gridModesMovedSrcs.erase(
        std::find(redistData.gridModesMovedSrcs.begin(),
                  redistData.gridModesMovedSrcs.end(), modesToMoveSrcs[i]));
    redistData.gridModesMovedSinks.erase(
        std::find(redistData.gridModesMovedSinks.begin(),
                  redistData.gridModesMovedSinks.end(), modesToMoveSinks[i]));
  }

  TensorDistribution gridModesMovedStartDist =
      GetTensorDistForGridModes(startDist, modesToMove);
  TensorDistribution gridModesMovedFinalDist =
      GetTensorDistForGridModes(finalDist, modesToMove);
...
template <typename T>
void DistTensor<T>::CommRedistP2P(const TensorDistribution &finalDist,
                                  const TensorDistribution &startDist,
                                  RedistPlanInfo &redistData,
                                  std::vector<Redist> &intDists) {
  Unsigned i;
  ...
  ModeArray movedModesSrcs =
      GetModeDistOfGridMode(redistData.gridModesMoved, startDist);
  ModeArray movedModesSinks =
      GetModeDistOfGridMode(redistData.gridModesMoved, finalDist);
...
mMoved, 2);
  ...
  if (AnyElemwiseNotEqual(maxLoc, startLoc)) {
    ...
    TensorDistribution movedModesStartDist =
        GetTensorDistForGridModes(startDist, movedModesDist);
    TensorDistribution movedModesFinalDist =
        GetTensorDistForGridModes(finalDist, movedModesDist);
    ...
    for (i = movedModesDist.Entries().size() - 1;
         i < movedModesDist.Entries().size(); i--) {
      Unsigned index = IndexOf(redistData.gridModesMoved, movedModesDist[i]);
      redistData.gridModesMoved.erase(redistData.gridModesMoved.begin() +
                                      index);
      redistData.gridModesMovedSrcs.erase(
          redistData.gridModesMovedSrcs.begin() + index);
      redistData.gridModesMovedSinks.erase(
          redistData.gridModesMovedSinks.begin() + index);
    }
...
template <typename T>
void DistTensor<T>::CommRedist(const TensorDistribution &finalDist,
                               const TensorDistribution &startDist,
                               RedistPlanInfo &redistData,
                               std::vector<Redist> &intDists) {
  ...
  if (finalDist == startDist) {
    ...
    // if(commRank == 0)
    // std::cout << "Add: " << TensorDistToString(finalDist) << " <-- " <<
    // TensorDistToString(startDist) << std::endl;
    CommRedistAdd(finalDist, startDist, redistData, intDists);
  } else if (redistData.tenModesReduced.size() > 0) {
    // if(commRank == 0)
    // std::cout << "Reduce: " << TensorDistToString(finalDist) << " <-- " <<
    // TensorDistToString(startDist) << std::endl;
  ...
  // Try the Perm first
    ...
    // if(commRank == 0)
    // std::cout << "P2P: " << TensorDistToString(finalDist) << " <-- " <<
    // TensorDistToString(startDist) << std::endl;
  ...
  // Finally, perform the necessary replication
    ...
    // if(commRank == 0)
    // std::cout << "Rmv: " << TensorDistToString(finalDist) << " <-- " <<
    // TensorDistToString(startDist) << std::endl;
  ...
  // All thats left is a shuffle
    ...
    //		printf("failure\n");
    //		if(commRank == 0)
    //			std::cout << "Final: " << TensorDistToString(finalDist) << " <-- "
    //<< TensorDistToString(startDist) << std::endl;  Add the final
    // redistribution  NOTE: modes is clearly a hack until interface changes
    Redist redistInfo;
