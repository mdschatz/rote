...
template <typename T>
void DistTensor<T>::ReduceUpdateRedistFrom(const RedistType &redistType,
                                           const T alpha,
  ...
  for (i = 0; i < sortedRModes.size(); i++) {
    ...
    if (rMode == tmp2Strides.size()) {
      if (rMode == 0) {
        ...
        tmp2Strides.insert(tmp2Strides.begin() + rMode,
                           tmp2Strides[tmp2Strides.size() - 1] *
                               tmp2Shape[tmp2Shape.size() - 1]);
      }
    } else {
  ...
  if (alpha == T(0))
    ...
    if (ElemwiseLessThanEqualTo(
            FilterVector(A.Shape(), sortedRModes),
            FilterVector(A.GridViewShape(), sortedRModes))) {
  ...
  ModeArray commModes =
      A.TensorDist().Filter(sortedRModes).UsedModes().Entries();
  ...
  switch (redistType) {
  case RS:
    tmp2.ReduceScatterUpdateCommRedist(alpha, tmp, beta, sortedRModes,
                                       commModes);
    break;
  case RTO:
    tmp2.ReduceToOneUpdateCommRedist(alpha, tmp, beta, commModes);
    break;
  case AR:
    tmp2.AllReduceUpdateCommRedist(alpha, tmp, beta, commModes);
    break;
  default:
    break;
  }
