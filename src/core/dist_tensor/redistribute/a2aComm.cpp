...
template <typename T>
void DistTensor<T>::AllToAllCommRedist(const DistTensor<T> &A,
  ...
  const std::vector<Unsigned> localPackStrides =
      ElemwiseDivide(LCMs(gvBShape, gvAShape), gvAShape);
...
template <typename T>
void DistTensor<T>::PackA2ACommSendBuf(const DistTensor<T> &A,
                                       const ModeArray &commModes,
                                       const ObjShape &sendShape,
                                       T *const sendBuf) {
  ...
  const std::vector<Unsigned> modeStrideFactor =
      ElemwiseDivide(commLCMs, gvAShape);
  ...
  const Unsigned nRedistProcsAll =
      Max(1, prod(FilterVector(gridShape, commModes)));
  ...
  // For each process we send to, we need to determine the first element we need
  // to send them
  PARALLEL_FOR
  for (Unsigned i = 0; i < nRedistProcsAll; i++) {
    Unsigned j;
    // Invert the process order based on the communicator used, to the actual
    // process location
    ...
    std::vector<Unsigned> alignBinA = GridLoc2ParticipatingGridViewLoc(
        firstOwnerB, g.Shape(), A.TensorDist());
    Location sendGridLoc = GridViewLoc2GridLoc(
        A.DetermineOwnerNewAlignment(myFirstElemLocA, alignBinA), gvA);
    ...
    Location myFirstLoc =
        A.DetermineFirstElem(A.GetGridView().ParticipatingLoc());
    Location procFirstLoc =
        this->DetermineFirstElem(GridLoc2ParticipatingGridViewLoc(
            procGridLoc, gridShape, this->TensorDist()));
    ...
    for (j = 0; j < myFirstLoc.size(); j++) {
      ...
      while (myFirstIndex != sendFirstIndex &&
             myFirstIndex < this->Dimension(j)) {
    ...
    if (found && ElemwiseLessThan(firstSendLoc, this->Shape())) {
      ...
      Unsigned dataBufPtr = LinearLocFromStrides(
          PermuteVector(localLoc, A.localPerm_), A.LocalStrides());

      PackData packData;
      packData.srcBufStrides = ElemwiseProd(
          A.LocalStrides(), PermuteVector(modeStrideFactor, A.localPerm_));
      ...
      Permutation out2in =
          A.localPerm_.PermutationTo(this->localPerm_)
              .InversePermutation(); // DetermineInversePermutation(DeterminePermutation(A.localPerm_,
                                     // this->localPerm_));

      // Permute pack strides to match input local permutation (for correct
      // packing)
      packData.dstBufStrides = PermuteVector(finalStrides, out2in);

      packData.loopShape =
          MaxLengths(ElemwiseSubtract(A.LocalShape(),
                                      PermuteVector(localLoc, A.localPerm_)),
                     PermuteVector(modeStrideFactor, A.localPerm_));

      PackCommHelper(packData, &(dataBuf[dataBufPt
r]),
                     &(sendBuf[i * nElemsPerProc]));
    }
  }
}

template <typename T>
void DistTensor<T>::UnpackA2ACommRecvBuf(const T *const recvBuf,
                                         const ModeArray &commModes,
                                         const ObjShape &recvShape,
  ...
  // For each process we recv from, we need to determine the first element we
  // get from them
  PARALLEL_FOR
  for (Unsigned i = 0; i < nRedistProcsAll; i++) {
    Unsigned j;
    ...
    // Get my first elem location
    Location myFirstLoc =
        this->DetermineFirstElem(this->GetGridView().ParticipatingLoc());

    // Determine what grid location p_i corresponds to BEFORE alignment has been
    // performed  so we correctly determine what elements to unpack
    Location firstOwnerB = GridViewLoc2GridLoc(this->Alignments(), gvB);
    Location unpackProcGVA = GridLoc2ParticipatingGridViewLoc(
        procGridLoc, g.Shape(), A.TensorDist());
    std::vector<Unsigned> alignBinA = GridLoc2ParticipatingGridViewLoc(
        firstOwnerB, g.Shape(), A.TensorDist());
    Location myFirstElemLocAligned =
        A.DetermineFirstUnalignedElem(unpackProcGVA, alignBinA);
    Location procLocBeforeRealign =
        GridViewLoc2GridLoc(A.DetermineOwner(myFirstElemLocAligned), gvA);

    Location procFirstLoc =
        A.DetermineFirstElem(GridLoc2ParticipatingGridViewLoc(
            procLocBeforeRealign, gridShape, A.TensorDist()));
    ...
    for (j = 0; j < myFirstLoc.size(); j++) {
      ...
      while (myFirstIndex != recvFirstIndex &&
             myFirstIndex < this->Dimension(j)) {
    ...
    if (found && ElemwiseLessThan(firstRecvLoc, this->Shape())) {
      ...
      Unsigned dataBufPtr = LinearLocFromStrides(
          PermuteVector(localLoc, this->localPerm_), this->LocalStrides());

      PackData unpackData;
      unpackData.dstBufStrides =
          ElemwiseProd(this->LocalStrides(),
                       PermuteVector(modeStrideFactor, this->localPerm_));
      ...
      // Test to fix bug
      unpackData.loopShape = MaxLengths(
          ElemwiseSubtract(this->LocalShape(),
                           PermuteVector(localLoc, this->localPerm_)),
          PermuteVector(modeStrideFactor, this->localPerm_));

      if (alpha == T(0))
        PackCommHelper(unpackData, &(recvBuf[i * nElemsPerProc]),
                       &(dataBuf[dataBufPtr]));
      else {
        YAxpByData data;
        ...
        YAxpBy_fast(T(1), alpha, &(recvBuf[i * nElemsPerProc]),
                    &(dataBuf[dataBufPtr]), data);
      }
    }
  }
}
