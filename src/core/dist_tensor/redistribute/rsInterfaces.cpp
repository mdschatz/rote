...
template <typename T>
Int DistTensor<T>::CheckPartialReduceScatterRedist(const DistTensor<T>& A, const Mode reduceScatterMode){
    Unsigned i;
    ...
    if(this->Dimension(reduceScatterMode) != Max(1,MaxLength(A.Dimension(reduceScatterMode), gvA.Dimension(reduceScatterMode))))
...
template <typename T>
Int DistTensor<T>::CheckReduceScatterRedist(const DistTensor<T>& A, const Mode reduceMode, const Mode scatterMode){
//    Unsigned i;
//    const tmen::GridView gvA = A.GridView();
//
...
template <typename T>
void DistTensor<T>::PartialReduceScatterRedist(const DistTensor<T>& A, const Mode reduceScatterMode){
    if(!CheckPartialReduceScatterRedist(A, reduceScatterMode))
...
template <typename T>
void DistTensor<T>::ReduceScatterRedist(const DistTensor<T>& A, const Mode reduceMode, const Mode scatterMode){
    if(!CheckReduceScatterRedist(A, reduceMode, scatterMode))
...
#define PROTO(T) \
        template Int DistTensor<T>::CheckPartialReduceScatterRedist(const DistTensor<T>& A, const Mode reduceScatterMode); \
        template Int DistTensor<T>::CheckReduceScatterRedist(const DistTensor<T>& A, const Mode reduceMode, const Mode scatterMode); \
        template void DistTensor<T>::PartialReduceScatterRedist(const DistTensor<T>& A, const Mode reduceScatterMode); \
        template void DistTensor<T>::ReduceScatterRedist(const DistTensor<T>& A, const Mode reduceMode, const Mode scatterMode);
