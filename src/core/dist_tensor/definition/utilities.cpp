...
template <typename T> void DistTensorBase<T>::ComplainIfReal() const {
  if (!IsComplex<T>::val)
...
// TODO: FIX THIS CHECK
template <typename T>
Location DistTensorBase<T>::DetermineOwner(const Location &loc) const {
#ifndef RELEASE
...
template <typename T>
Location DistTensorBase<T>::DetermineOwnerNewAlignment(
    const Location &loc, std::vector<Unsigned> &newAlignment) const {
#ifndef RELEASE
...
template <typename T>
Location DistTensorBase<T>::Global2LocalIndex(const Location &globalLoc) const {
#ifndef RELEASE
  ...
  for (i = 0; i < globalLoc.size(); i++) {
    localLoc[i] =
        (globalLoc[i] - ModeShift(i) + ModeAlignment(i)) / ModeStride(i);
  }
  return localLoc;
}

template <typename T>
mpi::Comm DistTensorBase<T>::GetCommunicatorForModes(const ModeArray &commModes,
  ...
  if (commMap_->count(sortedCommModes) == 0) {
    ...
    const Unsigned commColor =
        Loc2LinearLoc(gridSliceNegLoc, gridSliceNegShape);
...
template <typename T> void DistTensorBase<T>::SetParticipatingComm() {
#ifndef RELEASE
...
template <typename T>
void DistTensorBase<T>::CopyLocalBuffer(const DistTensorBase<T> &A) {
#ifndef RELEASE
...
template <typename T> void DistTensorBase<T>::ClearCommMap() {
#ifndef RELEASE
...
template <typename T> Unsigned DistTensorBase<T>::CommMapSize() {
#ifndef RELEASE
  CallStackEntry cse("DistTensor::ClearCommMap");
#endif
...
template <typename T>
Location DistTensorBase<T>::DetermineFirstUnalignedElem(
    const Location &gridViewLoc,
    const std::vector<Unsigned> &alignmentDiff) const {
#ifndef RELEASE
...
template <typename T>
bool DistTensor<T>::AlignCommBufRedist(const DistTensor<T> &A,
                                       const T *unalignedSendBuf,
  ...
  std::vector<Unsigned> alignBinA =
      GridLoc2ParticipatingGridViewLoc(firstOwnerB, g.Shape(), A.TensorDist());

  Location alignedFirstOwnerA =
      GridLoc2GridViewLoc(firstOwnerB, g.Shape(), A.TensorDist());
  Location myFirstElemLocA = A.DetermineFirstElem(gvA.ParticipatingLoc());
  Location myFirstElemLocAligned =
      A.DetermineFirstUnalignedElem(gvA.ParticipatingLoc(), alignBinA);

  Location sendGridLoc = GridViewLoc2GridLoc(
      A.DetermineOwnerNewAlignment(myFirstElemLocA, alignBinA), gvA);
  Location recvGridLoc =
      GridViewLoc2GridLoc(A.DetermineOwner(myFirstElemLocAligned), gvA);

  // Create the communicator to involve all processes we need to fix
  // misalignment
  ...
  mpi::SendRecv(unalignedSendBuf, sendSize, sendLinLoc, alignedSendBuf,
                recvSize, recvLinLoc, sendRecvComm);
  return true;
}

template <typename T>
ModeArray DistTensorBase<T>::GetMisalignedModes(const DistTensor<T> &B) const {
  // DEBUG
...
#define FULLBASE(T) PROTOBASE(T);

#define PROTO(T) template class DistTensor<T>
#define COPY(T) template DistTensor<T>::DistTensor(const DistTensor<T> &A)
#define FULL(T) PROTO(T);
