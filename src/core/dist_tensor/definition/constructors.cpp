...
DistTensorBase<T>::DistTensorBase(const rote::Grid &grid)
    : dist_(), shape_(),
      ...
      constrainedModeAlignments_(), modeAlignments_(), modeShifts_(),

      tensor_(true), localPerm_(),

      grid_(&grid), commMap_(&(DefaultCommMap())), gridView_(grid_, dist_),
      participatingComm_(),

      viewType_(OWNER), auxMemory_() {
  SetShifts();
...
DistTensorBase<T>::DistTensorBase(const Unsigned order, const rote::Grid &grid)
    : dist_(order + 1), shape_(order, 0),

      constrainedModeAlignments_(order, 0), modeAlignments_(order, 0),
      modeShifts_(order, 0),

      tensor_(order, false), localPerm_(order),
...
template <typename T>
DistTensorBase<T>::DistTensorBase(const TensorDistribution &dist,
                                  const rote::Grid &grid)
    : dist_(dist), shape_(dist_.size() - 1, 0),

      constrainedModeAlignments_(shape_.size(), 0),
      modeAlignments_(shape_.size(), 0), modeShifts_(shape_.size(), 0),

      tensor_(shape_.size(), false), localPerm_(shape_.size()),
...
template <typename T>
DistTensorBase<T>::DistTensorBase(const ObjShape &shape,
                                  const TensorDistribution &dist,
                                  const rote::Grid &grid)
    : dist_(dist), shape_(shape),
...
DistTensorBase<T>::DistTensorBase(const ObjShape &shape,
    ...
    : dist_(dist), shape_(shape),
      ...
      grid_(&g), commMap_(&(DefaultCommMap())), gridView_(grid_, dist_),
      participatingComm_(),
...
DistTensorBase<T>::DistTensorBase(const ObjShape &shape,
  ...
  SetDefaultPermutation();
  LockedAttach(shape, modeAlignments, buffer, strides, g);
  SetParticipatingComm();
}
...
DistTensorBase<T>::DistTensorBase(const ObjShape &shape,
  ...
  SetDefaultPermutation();
  Attach(shape, modeAlignments, buffer, strides, g);
  SetParticipatingComm();
}
...
template <typename T>
DistTensorBase<T>::DistTensorBase(const std::string &dist,
                                  const rote::Grid &grid)
    : dist_(StringToTensorDist(dist)), shape_(dist_.size() - 1, 0),
...
DistTensorBase<T>::DistTensorBase(const ObjShape &shape,
                                  ...
                                  const rote::Grid &grid)
    : dist_(StringToTensorDist(dist)), shape_(shape),
...
DistTensorBase<T>::DistTensorBase(const ObjShape &shape,
                                  ...
                                  const rote::Grid &g)
    : dis
t_(StringToTensorDist(dist)), shape_(shape),
...
DistTensorBase<T>::DistTensorBase(const DistTensorBase<T> &A)
    : dist_(), shape_(),
      ...
      tensor_(true), localPerm_(shape_.size()),

      grid_(&(A.Grid())), commMap_(&(DefaultCommMap())),
      gridView_(grid_, dist_), participatingComm_(),
...
template <typename T> DistTensorBase<T>::~DistTensorBase() {}

template <typename T> void DistTensorBase<T>::Swap(DistTensorBase<T> &A) {
...
template <typename T> rote::DistData DistTensorBase<T>::DistData() const {
  rote::DistData data;
  // data.colDist = MC;
  // data.rowDist = MR;
...
template <typename T>
const DistTensorBase<T> &DistTensorBase<T>::
operator=(const DistTensorBase<T> &A) {
#ifndef RELEASE
  ...
  if (Grid() == A.Grid()) {
    if (A.Order() != Order()) {
      ...
      //            localPerm_.resize(shape_.size()); //Resizing handled in
      //            operator=
    }
    ResizeTo(A);
