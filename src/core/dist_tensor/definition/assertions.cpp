...
#ifndef RELEASE
template <typename T> void DistTensorBase<T>::AssertNotLocked() const {
  if (Locked())
    LogicError("Assertion that tensor not be a locked view failed");
}

template <typename T> void DistTensorBase<T>::AssertNotStoringData() const {
  if (tensor_.MemorySize() > 0)
    LogicError("Assertion that tensor not be storing data failed");
}

template <typename T>
void DistTensorBase<T>::AssertValidEntry(const Location &loc) const {
#ifndef RELEASE
...
// TODO: FIX ASSERTIONS
template <typename T>
void DistTensorBase<T>::AssertValidSubtensor(const Location &loc,
                                             const ObjShape &shape) const {
  const Unsigned order = Order();
  if (shape.size() != order)
...
template <typename T>
void DistTensorBase<T>::AssertSameGrid(const rote::Grid &grid) const {
  if (Grid() != grid)
    LogicError("Assertion that grids match failed");
}

template <typename T>
void DistTensorBase<T>::AssertSameSize(const ObjShape &shape) const {
  const Unsigned order = Order();
  if (shape.size() != order)
...
template <typename T>
void DistTensorBase<T>::AssertMergeableModes(
...
void AssertConforming2x1(const DistTensor<T> &AT, const DistTensor<T> &AB,
  ...
  if (AnyElemwiseNotEqual(NegFilterVector(AT.Shape(), negFilterAT),
                          NegFilterVector(AB.Shape(), negFilterAB))) {
    Unsigned i;
    std::ostringstream msg;
  ...
  if (AnyElemwiseNotEqual(NegFilterVector(AT.Alignments(), negFilterAT),
                          NegFilterVector(AB.Alignments(), negFilterAB)))
...
#define CONFORMING(T)                                                          \
  template void AssertConforming2x1(const DistTensor<T> &AT,                   \
                                    const DistTensor<T> &AB, Mode mode);
