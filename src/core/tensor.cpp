...
template <typename T>
void Tensor<T>::AssertValidDimensions(
    const ObjShape &shape, const std::vector<Unsigned> &strides) const {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::AssertValidEntry(const Location &loc) const {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::AssertMergeableModes(
    const std::vector<ModeArray> &oldModes) const {
#ifndef RELEASE
...
Tensor<T>::Tensor(bool fixed)
    : shape_(), strides_(), viewType_(fixed ? OWNER_FIXED : OWNER), data_(0),
      memory_() {
...
Tensor<T>::Tensor(const Unsigned order, bool fixed)
    : shape_(order, 0), strides_(order, 1),
      viewType_(fixed ? OWNER_FIXED : OWNER), data_(0), memory_() {
  data_ = memory_.Buffer();
}
...
// TODO: Check for valid set of indices
template <typename T>
Tensor<T>::Tensor(const ObjShape &shape, const std::vector<Unsigned> &strides,
                  bool fixed)
    : shape_(shape), strides_(strides), viewType_(fixed ? OWNER_FIXED : OWNER) {
#ifndef RELEASE
  CallStackEntry cse("Tensor::Tensor");
...
template <typename T>
Tensor<T>::Tensor(const ObjShape &shape, const T *buffer,
                  const std::vector<Unsigned> &strides, bool fixed)
    : shape_(shape), strides_(strides),
      viewType_(fixed ? LOCKED_VIEW_FIXED : LOCKED_VIEW), data_(buffer),
      memory_() {
#ifndef RELEASE
  CallStackEntry cse("Tensor::Tensor");
#endif
...
template <typename T>
Tensor<T>::Tensor(const ObjShape &shape, T *buffer,
                  const std::vector<Unsigned> &strides, bool fixed)
    : shape_(shape), strides_(strides), viewType_(fixed ? VIEW_FIXED : VIEW),
...
Tensor<T>::Tensor(const Tensor<T> &A)
    : shape_(), strides_(), viewType_(OWNER), data_(0), memory_() {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::Swap(Tensor<T> &A) {
...
template <typename T> Unsigned Tensor<T>::Order() const {
  return shape_.size();
}

template <typename T> ObjShape Tensor<T>::Shape() const { return shape_; }

template <typename T> Unsigned Tensor<T>::Dimension(Mode mode) const {
  const Unsigned order = Order();
  if (mode >= order) {
...
template <typename T> std::vector<Unsigned> Tensor<T>::Strides() const {
  return strides_;
}

template <typename T> Unsigned Tensor<T>::Stride(Mode mode) const {
  const Unsigned order = Order();
  if (mode >= order) {
    LogicError("Requested mode dimension out of range.");
    return 0;
...
template <typename T> Unsigned Tensor<T>::MemorySize() const {
  return memory_.Size();
}

template <typename T> bool Tensor<T>::Owner() const {
  return IsOwner(viewType_);
}

template <typename T> bool Tensor<T>::Viewing() const {
  return !IsOwner(viewType_);
}

template <typename T> bool Tensor<T>::Shrinkable() const {
  return IsShrinkable(viewType_);
}

template <typename T> bool Tensor<T>::FixedSize() const {
  return !IsShrinkable(viewType_);
}

template <typename T> bool Tensor<T>::Locked() const {
  return IsLocked(viewType_);
}

template <typename T> T *Tensor<T>::Buffer() {
#ifndef RELEASE
...
template <typename T> const T *Tensor<T>::LockedBuffer() const { return data_; }

template <typename T> T *Tensor<T>::Buffer(const Location &loc) {
#ifndef RELEASE
  CallStackEntry cse("Tensor::Buffer");
  if (Locked())
    LogicError("Cannot return non-const buffer of locked Tensor");
...
template <typename T>
const T *Tensor<T>::LockedBuffer(const Location &loc) const {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::RemoveUnitModes(const ModeArray &modes) {
#ifndef RELEASE
  CallStackEntry cse("Tensor::LockedBuffer");
#endif
  Unsigned i;
...
template <typename T>
void Tensor<T>::IntroduceUnitModes(const ModeArray &modes) {
#ifndef RELEASE
  ...
  for (i = 0; i < sorted.size(); i++) {
    ...
    if (sorted[i] == strides_.size()) {
      if (sorted[i] == 0) {
        ...
        newStrideVal = strides_[shape_.size() - 1] * shape_[sha
pe_.size() - 1];
      }
    } else {
...
template <typename T> const T &Tensor<T>::Get_(const Location &loc) const {
  return data_[LinearLocFromStrides(loc, strides_)];
}

template <typename T> T &Tensor<T>::Set_(const Location &loc) {
  // NOTE: This const_cast has been carefully considered and should be safe
  //       since the underlying data should be non-const if this is called.
  return (const_cast<T *>(data_))[LinearLocFromStrides(loc, strides_)];
}

template <typename T> T Tensor<T>::Get(const Location &loc) const {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::Set(const Location &loc, T alpha) {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::Update(const Location &loc, T alpha) {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::ComplainIfReal() const {
  if (!IsComplex<T>::val)
...
template <typename T>
void Tensor<T>::SetRealPart(const Location &loc, BASE(T) alpha) {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::SetImagPart(const Location &loc, BASE(T) alpha) {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::UpdateRealPart(const Location &loc, BASE(T) alpha) {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::UpdateImagPart(const Location &loc, BASE(T) alpha) {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::Attach_(const ObjShape &shape, T *buffer,
...
template <typename T>
void Tensor<T>::Attach(const ObjShape &shape, T *buffer,
                       const std::vector<Unsigned> &strides) {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::LockedAttach_(const ObjShape &shape, const T *buffer,
                              const std::vector<Unsigned> &strides) {
  memory_.Empty();
  shape_ = shape;
  strides_ = strides;
...
template <typename T>
void Tensor<T>::LockedAttach(const ObjShape &shape, const T *buffer,
                             const std::vector<Unsigned> &strides) {
#ifndef RELEASE
...
template <typename T>
const Tensor<T> &Tensor<T>::operator=(const Tensor<T> &A) {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::Empty_() {
...
template <typename T> void Tensor<T>::Empty() {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::ResizeTo_(const ObjShape &shape) {
  ResizeTo_(shape, Dimensions2Strides(shape));
}

template <typename T> void Tensor<T>::ResizeTo(const Tensor<T> &A) {
#ifndef RELEASE
...
template <typename T> void Tensor<T>::ResizeTo(const ObjShape &shape) {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::ResizeTo_(const ObjShape &shape,
                          const std::vector<Unsigned> &strides) {
  Unsigned oldOrder = shape_.size();
  Unsigned curMaxElem =
      (oldOrder == 0) ? 1 : shape_[oldOrder - 1] * strides_[oldOrder - 1];

  Unsigned newOrder = shape.size();
  Unsigned newMaxElem =
      (newOrder == 0) ? 1 : shape[newOrder - 1] * strides[newOrder - 1];

  shape_ = shape;
  strides_ = strides;

  // Guard has newOrder == 0 in case we resize scalar to scalar (ensures at
  // least one data entry is created)
...
template <typename T>
void Tensor<T>::ResizeTo(const ObjShape &shape,
                         const std::vector<Unsigned> &strides) {
// TODO: IMPLEMENT CORRECTLY
  ...
  AssertValidDimensions(shape, strides);
  if (FixedSize() && (AnyElemwiseNotEqual(shape, shape_) ||
                      AnyElemwiseNotEqual(strides, strides_)))
    LogicError("Cannot change the size of this tensor");
  if (Viewing() && (AnyElemwiseNotEqual(shape, shape_) ||
                    AnyElemwiseNotEqual(strides, strides_)))
...
template <typename T> void Tensor<T>::CopyBuffer(const Tensor<T> &A) {
#ifndef RELEASE
...
template <typename T>
void Tensor<T>::CopyBuffer(const Tensor<T> &A, const Permutation &srcPerm,
                           const Permutation &dstPerm) {
#ifndef RELEASE
  CallStackEntry cse("Tenso
r::CopyBuffer");
#endif
  const T *srcBuf = A.LockedBuffer();
  T *thisBuf = Buffer();

  Permutation out2inPerm =
      dstPerm.PermutationTo(srcPerm); // DeterminePermutation(dstPerm, srcPerm);
  PackData packData;
