...
std::vector<Unsigned>
MaxLengths(const ObjShape& objShape, const ObjShape& wrapShape)
{
#ifndef RELEASE
    if(wrapShape.size() != objShape.size())
        LogicError("objShape order and wrapShape order must be the same");
    if( AnyZeroElem(wrapShape) )
        LogicError("wrapShape entries must be positive");
#endif
  std::vector<Unsigned> ret(objShape.size());
  for(Unsigned i = 0; i < ret.size(); i++)
      ret[i] = MaxLength(objShape[i], wrapShape[i]);
  return ret;
};
...
Location
LinearLoc2Loc(Unsigned linearLoc, const ObjShape& objShape)
{
    if(objShape.size() == 0 && linearLoc != 0)
        LogicError("Combination of linearLoc=0 and strides incompatible");
    const Unsigned order = objShape.size();
    Unsigned remainder = linearLoc;
    const std::vector<Unsigned> strides = Dimensions2Strides(objShape);
