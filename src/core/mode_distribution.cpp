...
ModeDistribution::ModeDistribution() : entries_() {}
...
Unsigned ModeDistribution::size() const { return entries_.size(); }

std::vector<Unsigned> ModeDistribution::Entries() const { return entries_; }

ModeDistribution operator+(const ModeDistribution &lhs,
                           const ModeDistribution &rhs) {
  ModeDistribution ret(lhs);
  ret.entries_.insert(ret.entries_.end(), rhs.entries_.begin(),
                      rhs.entries_.end());
  return ret;
}

ModeDistribution &ModeDistribution::operator=(const ModeDistribution &rhs) {
...
ModeDistribution &ModeDistribution::operator+=(const Mode &rhs) {
...
ModeDistribution operator-(const ModeDistribution &lhs,
  ...
  for (int i = 0; i < rhs.entries_.size(); i++) {
    ...
    auto loc =
        std::find(ret.entries_.begin(), ret.entries_.end(), modeToRemove);
...
bool IsPrefix(const ModeDistribution &lhs, const ModeDistribution &rhs) {
  Unsigned i;
...
bool operator<=(const ModeDistribution &lhs, const ModeDistribution &rhs) {
...
ModeDistribution::overlapWith(const ModeDistribution &rhs) const {
  ...
  for (i = 0; i < entries_.size(); i++)
    if (std::find(rhs.entries_.begin(), rhs.entries_.end(), entries_[i]) !=
        rhs.entries_.end())
      ret.entries_.push_back(entries_[i]);
  return ret;
}

bool ModeDistribution::SameModesAs(const ModeDistribution &rhs) const {
  Unsigned i;
...
bool ModeDistribution::Contains(const Mode &mode) const {
  return std::find(entries_.begin(), entries_.end(), mode) != entries_.end();
}

const Unsigned &ModeDistribution::operator[](size_t index) const {
...
ModeDistribution GetCommonSuffix(const ModeDistribution &lhs,
                                 const ModeDistribution &rhs) {
...
void ModeDistribution::CheckIsValid() {
...
inline std::string ModeDistToString_(const ModeDistribution &distribution,
                                     bool endLine) {
  std::stringstream ss;
  ss << "(";
...
inline std::string ModeDistToString(const ModeDistribution &distribution,
                                    bool endLine) {
  return ModeDistToString_(distribution, endLine);
}

inline ModeDistribution StringToModeDist(const std::string &s) {
