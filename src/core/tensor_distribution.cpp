...
TensorDistribution::TensorDistribution(Unsigned order)
: entries_(order + 1)
{ }

TensorDistribution::TensorDistribution(const std::vector<ModeDistribution>& dist)
: entries_(dist)
{ CheckIsValid(); }
...
Unsigned
TensorDistribution::size() const
{ return entries_.size(); }

std::vector<ModeDistribution>
TensorDistribution::Entries() const
{ return entries_; }
...
ModeDistribution
TensorDistribution::UnusedModes() const{
	return entries_[entries_.size() - 1];
}

void
TensorDistribution::SetToMatch(const TensorDistribution& other, const IndexArray& otherIndices, const IndexArray& myIndices){
	Unsigned i;
...
void
TensorDistribution::RemoveUnitModeDists(const std::vector<Unsigned>& unitModes){
	Unsigned i;
...
void
TensorDistribution::IntroduceUnitModeDists(const std::vector<Unsigned>& unitModes){
	Unsigned i;
	std::vector<Unsigned> sorted = unitModes;
	SortVector(sorted);
...
ModeDistribution
TensorDistribution::UsedModes() const{
	Unsigned i;
...
Mode
TensorDistribution::TensorModeForGridMode(const Mode& mode) const{
	Unsigned i;
	for(i = 0; i < entries_.size(); i++)
