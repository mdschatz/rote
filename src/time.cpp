...
bool Interval::operator<(const Interval &other) const { return dt < other.dt; }
...
double Interval::seconds() const { return dt; }

long long Interval::nflops() const { return flops; }

double Interval::gflops() const { return (double)flops / 1e9 / seconds(); }

double Interval::gmemops() const { return (double)memops / 1e9 / seconds(); }
  ...
  for (list<Timer>::iterator it = timers.begin(); it != timers.end(); ++it) {
    ...
    if (count > 0) {
      ...
      printf("%s:%*s %13.6f s %10ld x %11.6f gflops/sec x %11.6f gmemops/sec\n",
             it->name.c_str(), (int)(max_len - it->name.size()), "", tot, count,
             gflops, gmemops);
    }
  }
  if (found)
...
Interval toc() {
  ...
  if (!tics[tid]->empty())
    tics[tid]->back().flops -= dt.flops;
  if (!tics[tid]->empty())
    tics[tid]->back().memops -= dt.memops;
  return dt;
}
