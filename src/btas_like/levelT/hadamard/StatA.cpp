...
namespace rote{
	...
	template <typename T>
	void Hadamard<T>::runHelperStatAPartitionBC(Unsigned depth, BlkHadamardStatCInfo& hadamardInfo, const DistTensor<T>& A, const IndexArray& indicesA, const DistTensor<T>& B, const IndexArray& indicesB, DistTensor<T>& C, const IndexArray& indicesC){
		...
		while(B_T.Dimension(partModeB) < B.Dimension(partModeB)){
			...
			/*----------------------------------------------------------------*/
			std::cout <<"step 2\n";
			Hadamard<T>::runHelperStatAPartitionBC(depth+1, hadamardInfo, A, indicesA, B_1, indicesB, C_1, indicesC);
			count++;
...
// Exact copy from StatC.cpp
	template <typename T>
	void Hadamard<T>::runHelperStatAPartitionAC(
		...
		if(depth == hadamardInfo.partModesACA.size()){
			std::cout << "recur BC STAT A\n";
			Hadamard<T>::runHelperStatAPartitionBC(
				0, hadamardInfo,
				A, indicesA,
				B, indicesB,
				C, indicesC
		...
		while(A_T.Dimension(partModeA) < A.Dimension(partModeA)){
			...
			/*----------------------------------------------------------------*/
			Hadamard<T>::runHelperStatAPartitionAC(depth+1, hadamardInfo, A_1, indicesA, B, indicesB, C_1, indicesC);
			count++;
			/*----------------------------------------------------------------*/
