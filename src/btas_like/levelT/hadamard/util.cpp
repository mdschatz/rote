...
void Hadamard<T>::runHelperPartitionBC(
    Unsigned depth, BlkHadamardStatCInfo &hadamardInfo, const DistTensor<T> &A,
    const IndexArray &indicesA, const DistTensor<T> &B,
    const IndexArray &indicesB, DistTensor<T> &C, const IndexArray &indicesC) {
  if (depth == hadamardInfo.partModesBCB.size()) {
    if (hadamardInfo.isStatC) {
      ...
      intA.AlignModesWith(hadamardInfo.alignModesA, C,
                          hadamardInfo.alignModesATo);
      intA.RedistFrom(A);

      DistTensor<T> intB(hadamardInfo.distIntB, B.Grid());
      intB.AlignModesWith(hadamardInfo.alignModesB, C,
                          hadamardInfo.alignModesBTo);
      ...
      intC.AlignModesWith(hadamardInfo.alignModesC, A,
                          hadamardInfo.alignModesCTo);
      intC.RedistFrom(C);

      DistTensor<T> intB(hadamardInfo.distIntB, B.Grid());
      intB.AlignModesWith(hadamardInfo.alignModesB, A,
                          hadamardInfo.alignModesBTo);
      intB.SetLocalPermutation(hadamardInfo.permB);
      intB.RedistFrom(B);

      Hadamard<T>::run(A.LockedTensor(), indicesA, intB.LockedTensor(),
                       PermuteVector(indicesB, hadamardInfo.permB),
                       intC.Tensor(),
                       PermuteVector(indicesC, hadamardInfo.permC));
      C.RedistFrom(intC);
    }
    return;
  }
  ...
  while (B_T.Dimension(partModeB) < B.Dimension(partModeB)) {
    LockedRepartitionDown(B_T, B_0,
                          /**/ /**/
                          B_1, B_B, B_2, partModeB, blkSize);
    RepartitionDown(C_T, C_0,
                    /**/ /**/
                    C_1, C_B, C_2, partModeC, blkSize);

    /*----------------------------------------------------------------*/
    Hadamard<T>::runHelperPartitionBC(depth + 1, hadamardInfo, A, indicesA, B_1,
                                      indicesB, C_1, indicesC);

    count++;
    /*----------------------------------------------------------------*/
    SlideLockedPartitionDown(B_T, B_0, B_1,
                             /**/ /**/
                             B_B, B_2, partModeB);
    SlidePartitionDown(C_T, C_0, C_1,
                       /**/ /**/
                       C_B, C_2, partModeC);
  }
}
...
void Hadamard<T>::runHelperPartitionAC(
  ...
  if (depth == hadamardInfo.partModesACA.size()) {
    Hadamard<T>::runHelperPartitionBC(0, hadamardInfo, A, indicesA, B, indicesB,
                                      C, indicesC);
    return;
  }
  ...
  while (A_T.Dimension(partModeA) < A.Dimension(partModeA)) {
    LockedRepartitionDown(A_T, A_0,
                          /**/ /**/
                          A_1, A_B, A_2, partModeA, blkSize);
    RepartitionDown(C_T, C_0,
                    /**/ /**/
                    C_1, C_B, C_2, partModeC, blkSize);

    /*----------------------------------------------------------------*/
    Hadamard<T>::runHelperPartitionAC(depth + 1, hadamardInfo, A_1, indicesA, B,
                                      indicesB, C_1, indicesC);
    count++;
    /*----------------------------------------------------------------*/
    SlideLockedPartitionDown(A_T, A_0, A_1,
                             /**/ /**/
                             A_B, A_2, partModeA);
    SlidePartitionDown(C_T, C_0, C_1,
                       /**/ /**/
...
void Hadamard<T>::setHadamardInfo(
    const DistTensor<T> &A, const IndexArray &indicesA, const DistTensor<T> &B,
    const IndexArray &indicesB, const DistTensor<T> &C,
    const IndexArray &indicesC, const std::vector<Unsigned> &blkSizes,
    bool isStatC, BlkHadamardStatCInfo &hadamardInfo) {
  ...
  Unsigned i;
  IndexArray indicesCA =
      isStatC ? DiffVector(IsectVector(indicesC, indicesA), indicesB)
              : DiffVector(IsectVector(indicesA, indicesC), indicesB);
  IndexArray indicesCB = DiffVector(IsectVector(indicesC, indicesB), indicesA);
  IndexArray indicesCBA =
      isStatC ? IsectVector(IsectVector(indicesC, indicesB), indicesA)
  ...
  for (i = 0; i < indicesCA.size(); i++) {
    hadamardInfo.partModesACA[i] = IndexOf(indice
sA, indicesCA[i]);
  ...
  hadamardInfo.permA =
      DeterminePermutation(indicesA, ConcatenateVectors(indicesCA, indicesCBA));
  hadamardInfo.permB =
      DeterminePermutation(indicesB, ConcatenateVectors(indicesCBA, indicesCB));
  hadamardInfo.permC = DeterminePermutation(
      indicesC,
      ConcatenateVectors(ConcatenateVectors(indicesCBA, indicesCB), indicesCA));
}
