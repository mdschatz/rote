...
template <typename T>
void RecurContractStatA(Unsigned depth,
                        const BlkContractStatAInfo &contractInfo, T alpha,
                        const DistTensor<T> &A, const IndexArray &indicesA,
                        const DistTensor<T> &B, const IndexArray &indicesB,
                        T beta, DistTensor<T> &C, const IndexArray &indicesC) {
  if (depth == contractInfo.partModesB.size()) {
    ...
    // NOTE: Overwrites values, but this is correct (initially sets to match gvA
    // but then overwrites with C)
    ...
    intB.AlignModesWith(contractInfo.alignModesB, A,
                        contractInfo.alignModesBTo);
    intB.SetLocalPermutation(contractInfo.permB);
    intB.RedistFrom(B);

    intT.AlignModesWith(contractInfo.alignModesT, A,
                        contractInfo.alignModesTTo);
    intT.SetLocalPermutation(contractInfo.permT);
    intT.ResizeTo(shapeT);

    LocalContract(alpha, A.LockedTensor(), indicesA, false, intB.LockedTensor(),
                  indicesB, false, T(0), intT.Tensor(), indicesT, false);
  ...
  while (B_T.Dimension(partModeB) < B.Dimension(partModeB)) {
    ...
    /*----------------------------------------------------------------*/
    RecurContractStatA(depth + 1, contractInfo, alpha, A, indicesA, B_1,
                       indicesB, beta, C_1, indicesC);
    /*----------------------------------------------------------------*/
...
void SetBlkContractStatAInfo(
    const TensorDistribution &distT, const IndexArray &indicesT,
    const IndexArray &indicesA, const TensorDistribution &distIntB,
    const IndexArray &indicesB, const IndexArray &indicesC,
  ...
  contractInfo.permA =
      DeterminePermutation(indicesA, ConcatenateVectors(indicesAC, indicesAB));
  contractInfo.permB =
      DeterminePermutation(indicesB, ConcatenateVectors(indicesAB, indicesBC));
  contractInfo.permT = DeterminePermutation(
      indicesT,
      ConcatenateVectors(ConcatenateVectors(indicesAC, indicesBC), indicesAB));
}

template <typename T>
void ContractStatA(T alpha, const DistTensor<T> &A, const IndexArray &indicesA,
  ...
  for (i = 0; i < contractIndices.size(); i++) {
    ...
    if (index >= 0)
      reduceGridModes =
          ConcatenateVectors(reduceGridModes, distA[index].Entries());
  }
  ...
  distIntC.AppendToMatchForGridModes(reduceGridModes, distC, indicesC,
                                     indicesC);

  // Create the Contract Info
  BlkContractStatAInfo contractInfo;
  SetBlkContractStatAInfo(distT, indicesT, indicesA, distIntB, indicesB,
                          indicesC, blkSizes, contractInfo);
  ...
  RecurContractStatA(0, contractInfo, alpha, tmpA, indicesA, B, indicesB, beta,
                     C, indicesC);
}
...
#define PROTO(T)                                                               \
  template void ContractStatA(                                                 \
      T alpha, const DistTensor<T> &A,
