...
template <typename T>
inline void ZAxpBy_fast(T alpha, T beta, T const *const src1Buf,
                        T const *const src2Buf, T *const dstBuf,
                        const ZAxpByData &data) {
  ...
  if (alpha == T(1) && beta == T(1)) {
    ...
    while (!done) {
      ...
      dstBuf[dstBufPtr] =
          alpha * src1Buf[src1BufPtr] + beta * src2Buf[src2BufPtr];
      // Update
      curLoc[ptr]++;
      dstBufPtr += dstBufStrides[ptr];
      src1BufPtr += src1BufStrides[ptr];
...
template <typename T>
inline void ZAxpBy(T alpha, const Tensor<T> &X, T beta, const Tensor<T> &Y,
                   Tensor<T> &Z) {
#ifndef RELEASE
...
template <typename T>
inline void ZAxpBy(T alpha, const Tensor<T> &X, const Permutation &permXToZ,
...
template <typename T>
inline void ZAxpBy(T alpha, const DistTensor<T> &X, T beta,
                   const DistTensor<T> &Y, DistTensor<T> &Z) {
#ifndef RELEASE
  CallStackEntry entry("ZAxpBy");
  if (X.Grid() != Y.Grid())
  ...
  ZAxpBy(alpha, X.LockedTensor(), permXToZ, beta, Y.LockedTensor(), permYToZ,
         Z.Tensor());
}
...
#define PROTO(T)                                                               \
  template void ZAxpBy(T alpha, const DistTensor<T> &X, T beta,                \
                       const DistTensor<T> &Y, DistTensor<T> &Z);
