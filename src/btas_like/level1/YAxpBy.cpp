...
namespace rote {
template <typename T>
inline void YAxpBy_fast(T alpha, T beta, T const *const srcBuf, T *const dstBuf,
...
template <typename T>
inline void YAxpBy(T alpha, const Tensor<T> &X, const Permutation &permXToY,
                   T beta, Tensor<T> &Y) {
#ifndef RELEASE
...
template <typename T>
inline void YxpBy(const DistTensor<T> &X, T beta, DistTensor<T> &Y) {
  YAxpBy(T(1), X, beta, Y);
}

template <typename T>
inline void YAxpy(T alpha, const DistTensor<T> &X, DistTensor<T> &Y) {
  YAxpBy(alpha, X, T(1), Y);
}

template <typename T>
inline void Yxpy(const DistTensor<T> &X, DistTensor<T> &Y) {
  YAxpBy(T(1), X, T(1), Y);
}

template <typename T>
inline void YAxpBy(T alpha, const DistTensor<T> &X, T beta, DistTensor<T> &Y) {
#ifndef RELEASE
  CallStackEntry entry("Yxpy");
  if (X.Grid() != Y.Grid())
    LogicError("X and Y must be distributed over the same grid");
#endif
  Permutation permXToY =
      X.LocalPermutation().PermutationTo(Y.LocalPermutation());
...
#define PROTO(T)                                                               \
  ...
  template void
YAxpBy(T alpha, const DistTensor<T> &X, T beta,                \
                       DistTensor<T> &Y);                                      \
  template void YAxpBy_fast(T alpha, T beta, T const *const srcBuf,            \
                            T *const dstBuf, const YAxpByData &data);
