...
template <typename T>
void YAxpPx_fast(T alpha, T beta, T const *const srcBuf,
                 T const *const permSrcBuf, T *const dstBuf,
                 const YAxpPxData &data) {
  ...
  while (!done) {
    dstBuf[dstBufPtr] =
        alpha * srcBuf[srcBufPtr] + beta * permSrcBuf[permBufPtr];
...
template <typename T>
void YAxpPx(T alpha, const Tensor<T> &X, T beta, const Tensor<T> &PX,
            const Permutation &perm, Tensor<T> &Y) {
#ifndef RELEASE
...
template <typename T>
void YAxpPx(T alpha, const Tensor<T> &X, const Permutation &permXToY, T beta,
            const Tensor<T> &PX, const Permutation &permPXToY, Tensor<T> &Y) {
#ifndef RELEASE
  CallStackEntry entry("YAxpPx");
#endif
  YAxpPxData data;
...
void YAxpPx(T alpha, const DistTensor<T> &X, T beta, const DistTensor<T> &PX,
  ...
  YAxpPx(alpha, X.LockedTensor(), permXToY, beta, PX.LockedTensor(), permPXToY,
         Y.Tensor());
}
...
#define PROTO(T)                                                               \
  template void YAxpPx(T alpha, const DistTensor<T> &X, T beta,                \
