...
template <typename T>
inline void ZAxpBypPx_fast(T alpha, T beta, T const *const src1Buf,
                           T const *const src2Buf, T const *const permSrcBuf,
                           T *const dstBuf, const ZAxpBypPxData &data) {
  ...
  if (alpha == T(1) && beta == T(1)) {
    ...
    while (!done) {
      ...
      dstBuf[dstBufPtr] =
          src1Buf[src1BufPtr] + src2Buf[src2BufPtr] + permSrcBuf[permSrcBufPtr];
      // Update
      curLoc[ptr]++;
    ...
    while (!done) {
      ...
      dstBuf[dstBufPtr] = alpha * src1Buf[src1BufPtr] +
                          beta * src2Buf[src2BufPtr] +
                          permSrcBuf[permSrcBufPtr];
      // Update
      curLoc[ptr]++;
      dstBufPtr += dstBufStrides[ptr];
...
template <typename T>
inline void ZAxpBypPx(T alpha, const Tensor<T> &X, T beta, const Tensor<T> &Y,
...
template <typename T>
inline void ZAxpBypPx(T alpha, const Tensor<T> &X, const Permutation &permXToZ,
                      T beta, const Tensor<T> &Y, const Permutation &permYToZ,
                      const Tensor<T> &PX, const Permutation &permPXToZ,
                      Tensor<T> &Z) {
#ifndef RELEASE
...
template <typename T>
inline void ZAxpBypPx(T alpha, const DistTensor<T> &X, T beta,
                      const DistTensor<T> &Y, const DistTensor<T> &PX,
...
#endif
  Permutation permXToZ =
      X.LocalPermutation().PermutationTo(Z.LocalPermutation());
  Permutation permYToZ =
      Y.LocalPermutation().PermutationTo(Z.LocalPermutation());
  ...
  ZAxpBypPx(alpha, X.LockedTensor(), permXToZ, beta, Y.LockedTensor(), permYToZ,
            PX.LockedTensor(), permPXToZ, Z.Tensor());
}

// Non-template functions
// bool AnyFalseElem(const std::vector<bool>& vec);
#define PROTO(T)                                                               \
  template void ZAxpBypPx(T alpha, const Tensor<T> &X, T beta,                 \
                          const Tensor<T> &Y, const Tensor<T> &PX,
\
                          Tensor<T> &Z);                                       \
  template void ZAxpBypPx(T alpha, const DistTensor<T> &X, T beta,             \
                          const DistTensor<T> &Y, const DistTensor<T> &PX,     \
                          const Permutation &perm, DistTensor<T> &Z);
