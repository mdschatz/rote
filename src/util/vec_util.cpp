...
T sum(const std::vector<T> &src, const Unsigned startIndex) {
  if (src.size() == 0)
    return 0;
  return std::accumulate(src.begin() + startIndex, src.end(), T(0),
                         std::plus<T>());
}
...
T sum(const std::vector<T> &src, const Unsigned startIndex,
  ...
  if (src.size() == 0)
    return 0;
  return std::accumulate(src.begin() + startIndex, src.begin() + nElem, T(0),
                         std::plus<T>());
}
...
T prod(const std::vector<T> &src, const Unsigned startIndex) {
  if (src.size() == 0)
    return 0;
  return std::accumulate(src.begin() + startIndex, src.end(), T(1),
                         std::multiplies<T>());
}
...
T prod(const std::vector<T> &src, const Unsigned startIndex,
  ...
  if (src.size() == 0)
    return 0;
  return std::accumulate(src.begin() + startIndex, src.begin() + nElem, T(1),
                         std::multiplies<T>());
}
...
void ElemwiseSum(const std::vector<T> &src1, const std::vector<T> &src2,
                 std::vector<T> &out) {
  std::transform(src1.begin(), src1.end(), src2.begin(), out.begin(),
                 std::plus<T>());
}
...
#define PROTO(T)                                                               \
  ...
  template T prod(const std::vector<T> &src, const Unsigned startIndex,        \
                  const Unsigned endIndex);                                    \
  template void ElemwiseSum(const std::vector<T> &src1,                        \
                            const std::vector<T> &src2, std::vector<T> &out);  \
  template std::vector<T> ElemwiseSum(const std::vector<T> &src1,              \
                                      const std::vector<T> &src2);             \
  template void ElemwiseSubtract(const std::vector<T> &src1,                   \
                                 const std::vector<T> &src2,                   \
                                 std::vector<T> &out);                         \
  template std::vector<T> ElemwiseSubtract(const std::vector<T> &src1,         \
                                           const std::vector<T> &src2);        \
  template void ElemwiseProd(const std::vector<T> &src1,                       \
                             const std::vector<T> &src2, std::vector<T> &out); \
  template std::vector<T> ElemwiseProd(const std::vector<T> &src1,             \
                                       const std::vector<T> &src2);            \
  template void ElemwiseDivide(const std::vector<T> &src1,                     \
                               const std::vector<T> &src2,                     \
                               std::vector<T> &out);                           \
  template std::vector<T> ElemwiseDivide(const std::vector<T> &src1,           \
                                         const std::vector<T> &src2);          \
  ...
  template bool ElemwiseLessThan(const std::vector<T> &vec1,                   \
                                 const std::vector<T> &vec2);                  \
  template bool ElemwiseLessThanEqualTo(const std::vector<T> &vec1,            \
                                        const std::vector<T> &vec2);           \
  template bool AnyElemwiseGreaterThan(const std::vector<T> &vec1,             \
                                       const std::vector<T> &vec2);            \
  template bool AnyElemwiseGreaterThanEqualTo(const std::vector<T> &vec1,      \
                                              const std::vector<T> &vec2);     \
  template bool AnyZeroElem(const std::vector<T> &vec);                        \
  template bool AnyElemwiseNotEqual(const std::vector<T> &vec1,                \
                                    cons
t std::vector<T> &vec2);               \
  template bool EqualUnderPermutation(const std::vector<T> &vec1,              \
                                      const std::vector<T> &vec2);             \
  template std::vector<T> PermuteVector(const std::vector<T> &vec,             \
                                        const Permutation &filter);            \
  template std::vector<T> FilterVector(const std::vector<T> &vec,              \
                                       const std::vector<Unsigned> &filter);   \
  template std::vector<T> NegFilterVector(                                     \
      const std::vector<T> &vec, const std::vector<Unsigned> &filter);         \
  template bool IsSame(const std::vector<T> &vec1,                             \
                       const std::vector<T> &vec2);                            \
  template bool IsSuffix(const std::vector<T> &vec1,                           \
                         const std::vector<T> &vec2);                          \
  template bool IsPrefix(const std::vector<T> &vec1,                           \
                         const std::vector<T> &vec2);                          \
  template int IndexOf(const std::vector<T> &vec, const T &val);               \
  template std::vector<Unsigned> IndicesOf(const std::vector<T> &vec,          \
                                           const std::vector<T> &vals);        \
  template bool Contains(const std::vector<T> &vec, const T &val);             \
  template std::vector<T> Unique(const std::vector<T> &vec);                   \
  template std::vector<T> GetSuffix_(const std::vector<T> &vec1,               \
                                     const std::vector<T> &vec2);              \
  template std::vector<T> GetSuffix(const std::vector<T> &vec1,                \
                                    const std::vector<T> &vec2);               \
  template std::vector<T> GetPrefix_(const std::vector<T> &vec1,               \
                                     const std::vector<T> &vec2);              \
  template std::vector<T> GetPrefix(const std::vector<T> &vec1,                \
                                    const std::vector<T> &vec2);               \
  template std::vector<T> ConcatenateVectors(const std::vector<T> &vec1,       \
                                             const std::vector<T> &vec2);      \
  template T Min(const std::vector<T> &vec);                                   \
  template T Max(const std::vector<T> &vec);                                   \
  template Permutation DeterminePermutation(const std::vector<T> &ref,         \
                                            const std::vector<T> &vec);        \
  template Permutation DetermineSubPermutation(const std::vector<T> &ref,      \
                                               const std::vector<T> &vec);     \
  template std::vector<T> DiffVector(const std::vector<T> &vec1,               \
                                     const std::vector<T> &vec2);              \
  template std::vector<T> IsectVector(const std::vector<T> &vec1,              \
                                      const std::vector<T> &vec2);             \
...
#define PROTOMOD(T)                                                            \
  template void ElemwiseMod(const std::vector<T> &src1,                        \
                            const std::vector<T> &src2, std::vector<T> &out);  \
  template std::vector<T> ElemwiseMod(const std::vector<T> &src1,              \
                                      const std::vector<T> &src2);
PROTOMOD(Unsigned)
PROTOMOD(Int)
