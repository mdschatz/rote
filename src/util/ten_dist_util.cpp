...
bool CheckOrder(const Unsigned &outOrder, const Unsigned &inOrder) {
  if (outOrder != inOrder) {
    LogicError("Invalid redistribution: Objects being redistributed must be of "
               "same order");
  }
  return true;
}
...
bool CheckSameNonDist(const TensorDistribution &outDist,
  ...
  if (!outDist[nonDistMode].SameModesAs(inDist[nonDistMode])) {
    ...
    msg << "Invalid redistribution\n"
        << outDist << " <-- " << inDist << std::endl
        << "Output Non-distributed mode distribution must be same" << std::endl;
    LogicError(msg.str());
  }
  return true;
}
...
bool CheckSameCommModes(const TensorDistribution &outDist,
  ...
  if (!remainderIn.UsedModes().SameModesAs(remainderOut.UsedModes())) {
    ...
    msg << "Invalid redistribution\n"
        << outDist << " <-- " << inDist << std::endl
        << "Cannot determine modes communicated over" << std::endl;
    LogicError(msg.str());
  }
  return true;
}
...
bool CheckPartition(const TensorDistribution &outDist,
  ...
  for (i = 0; i < objOrder; i++) {
    if (inDist[i].size() != outDist[i].size()) {
      ...
      msg << "Invalid redistribution\n"
          << outDist << " <-- " << inDist << std::endl
          << "Cannot form partition" << std::endl;
      LogicError(msg.str());
    }
  }
  ...
  for (Unsigned i = 0; i < objOrder; i++) {
    if (!(inDist[i] <= outDist[i])) {
      ...
      msg << "Invalid redistribution:\n"
          << outDist << " <-- " << inDist << std::endl
          << "Input mode-" << i
          << " mode distribution must be prefix of output mode distribution"
          << std::endl;
      LogicError(msg.str());
    }
  }
  ...
  for (Unsigned i = 0; i < objOrder; i++) {
    if (!(outDist[i] <= inDist[i])) {
      ...
      msg << "Invalid redistribution\n"
          << outDist << " <-- " << inDist << std::endl
          << "Output mode-" << i
          << " mode distribution must be prefix of input mode distribution"
          << std::endl;
      LogicError(msg.str());
    }
  }
...
bool CheckSameGridViewShape(const ObjShape &outShape, const ObjShape &inShape) {
  if (AnyElemwiseNotEqual(outShape, inShape)) {
    std::stringstream msg;
    msg << "Invalid redistribution: Mode distributions must correspond to "
           "equal logical grid dimensions"
        << std::endl;
    LogicError(msg.str());
  }
  return true;
...
Mode GetModeDistOfGridMode(const Mode &mode,
                           const TensorDistribution &tenDist) {
  Unsigned i;
